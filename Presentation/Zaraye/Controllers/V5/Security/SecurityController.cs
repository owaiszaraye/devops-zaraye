...
namespace Zaraye.Controllers.V5.Security
{
    ...
    public class SecurityController : BaseApiController
    {
        ...
        public SecurityController(ICustomerRegistrationService customerRegistrationService,
        ...
        public async Task<IActionResult> PhoneNumberAuthenticationOtp(string phoneNumber)
        {
            ...
            if (user != null)
                return Ok(new { success = false, message = "Phone number already exist" });
            ...
            return Ok(new { success = true, message = await _localizationService.GetResourceAsync("Account.Customer.Otpgenerated.successfully"), otp });
        }
            ...
            if (registrationResult.Success)
            {
                ...
                switch (_customerSettings.UserRegistrationType)
                {
                    case UserRegistrationType.EmailValidation:
                        ...
                        //result
                        return Ok(new { success = true, message = await _localizationService.GetResourceAsync("Account.Register.Result.EmailValidation") });

                    case UserRegistrationType.AdminApproval:
                        return Ok(new { success = true, message = await _localizationService.GetResourceAsync("Account.Register.Result.AdminApproval") });
        ...
        public async Task<IActionResult> ChangePassword([FromBody] AccountApiModel.ChangePasswordApiModel model)
        {
            ...
            if (!await _customerService.IsRegisteredAsync(user))
                return Ok(new { success = false, message = "Not not registered" });
            ...
            if (changePasswordResult.Success)
                return Ok(new { success = true, message = await _localizationService.GetResourceAsync("Account.ChangePassword.Success") });

            return Ok(new { success = false, message = string.Join(",", changePasswordResult.Errors) });
        }
        ...
        public async Task<IActionResult> PasswordRecoverySend([FromBody] AccountApiModel.RequestPasswordApiModel model)
        {
            ...
            if (customer != null && customer.Active && !customer.Deleted)
            {
                ...
                return Ok(new { success = true, message = await _localizationService.GetResourceAsync("Account.PasswordRecovery.EmailHasBeenSent") });
            }
            else
            {
                return Ok(new { success = false, message = await _localizationService.GetResourceAsync("Account.PasswordRecovery.EmailNotFound") });
            }
        }
        ...
        public async Task<IActionResult> AccountDeactivate(string comment)
        {
            ...
            return Ok(new { success = true, message = await _localizationService.GetResourceAsync("Account.Customer.Deactivated.Success") });
        }
        ...
        public async Task<IActionResult> BuyerContractSendOTP(string email)
        {
            ...
            try
            {
                ...
                return Ok(new { success = true, message = "", generateOtp });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> BuyerContractVerifyOTP(string email, string otp)
        {
            ...
            try
            {
                ...
                return Ok(new { success = true, message = "Valid Otp" });
            }
            catch (Exception ex)
            {
                return Ok(new { success = false, message = ex.Message });
        ...
        public async Task<IActionResult> ChangeBuyerPassword([FromBody] ChangeBuyerPasswordApiModel model)
        {
            ...
            if (string.IsNullOrWhiteSpace(model.NewPassword) || string.IsNullOrWhiteSpace(model.ConfirmPassword))
                return Ok(new { success = false, message = "Passwords not mathed" });
            ...
            return Ok(new { success = false, message = string.Join(",", response.Errors) });
        }
