...
using Zaraye.Framework.Mvc.ModelBinding;
//using Zaraye.Framework.Validators;

namespace Zaraye.Framework.Mvc.Filters
{
    /// <summary>
    /// Represents filter attribute that adds a detailed validation message that a value cannot be empty
    /// </summary>
    public sealed class ZarayeNullValidationMessageAttribute : TypeFilterAttribute
    {
        #region Ctor
        ...
        /// </summary>
        public ZarayeNullValidationMessageAttribute() : base(typeof(NotNullValidationMessageFilter))
        {
        }
        ...
        private class NotNullValidationMessageFilter : IAsyncActionFilter
        {
            ...
            private async Task CheckNotNullValidationAsync(ActionExecutingContext context)
            {
                ...
                var model = context.ActionArguments.Values.OfType<BaseZarayeModel>().FirstOrDefault();
                if (model is null)
                    return;
