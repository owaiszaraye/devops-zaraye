...
//using Zaraye.Framework.Globalization;
using System;
using System.Linq;
using System.Threading.Tasks;

namespace Zaraye.Framework
{
    /// <summary>
    ...
    public partial class WebWorkContext : IWorkContext
    {
        ...
        {
            var cookieName = $"{ZarayeCookieDefaults.Prefix}{ZarayeCookieDefaults.CustomerCookie}";
        ...
        protected virtual void SetLanguageCookie(Language language)
        {
            ...
            //delete current cookie value
            var cookieName = $"{ZarayeCookieDefaults.Prefix}{ZarayeCookieDefaults.CultureCookie}";
            _httpContextAccessor.HttpContext.Response.Cookies.Delete(cookieName);
        ...
        public virtual async Task SetCurrentCustomerAsync(Customer customer = null)
        {
            if (customer == null)
            {
                ...
                if (_httpContextAccessor.HttpContext?.Request
                    ?.Path.Equals(new PathString($"/{ZarayeTaskDefaults.ScheduleTaskPath}"), StringComparison.InvariantCultureIgnoreCase)
                    ?? true)
                {
                ...
                if (customer != null && !customer.Deleted && customer.Active && !customer.RequireReLogin)
                {
                    ...
                    var impersonatedCustomerId = await _genericAttributeService
                        .GetAttributeAsync<int?>(customer, ZarayeCustomerDefaults.ImpersonatedCustomerIdAttribute);
