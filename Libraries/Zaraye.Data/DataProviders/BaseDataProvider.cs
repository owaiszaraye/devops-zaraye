...
using Zaraye.Data.DataProviders.Interceptors;
...
namespace Zaraye.Data.DataProviders
{
    public abstract partial class BaseDataProvider : IMappingEntityAccessor
    {
        ...
        protected static ConcurrentDictionary<Type, ZarayeEntityDescriptor> EntityDescriptors { get; } = new ConcurrentDictionary<Type, ZarayeEntityDescriptor>();
        ...
        public virtual void InitializeDatabase()
        {
            ...
            var targetAssembly = typeof(ZarayeDbStartup).Assembly;
        ...
        public virtual ZarayeEntityDescriptor GetEntityDescriptor(Type entityType)
        {
            return EntityDescriptors.GetOrAdd(entityType, t =>
            {
                ...
                return new ZarayeEntityDescriptor
                {
                    EntityName = tableName,
                    SchemaName = builder.Expression.SchemaName,
                    Fields = builder.Expression.Columns.Select(column => new ZarayeEntityFieldDescriptor
                    {
                        Name = column.Name,
        ...
        public virtual async Task<TEntity> InsertEntityAsync<TEntity>(TEntity entity) where TEntity : BaseEntity
        {
            ...
            if (tempId > 0)
            {
                ...
                if (baseNameCompatibility.TableNames.TryGetValue(typeof(TEntity), out var mappedTableName))
                    tableName = mappedTableName;
                else
                    tableName = typeof(TEntity).Name;
                
                var query = $"UPDATE `{tableName}` SET Id = {tempId} WHERE Id = {entity.Id}";
        ...
        public virtual async Task<int> ExecuteNonQueryAsync(string sql, params DataParameter[] dataParameters)
        {
            ...
            var data = await command.ExecuteAsync();

            //custom line added for data migration on 22June2023 from Adnan Hakimuddin
            await command.DataConnection.Connection.CloseAsync();
