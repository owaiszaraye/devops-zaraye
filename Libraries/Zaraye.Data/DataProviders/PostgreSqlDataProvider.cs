...
using Zaraye.Data.DataProviders.LinqToDB;
using Zaraye.Data.Migrations;
using Npgsql;

namespace Zaraye.Data.DataProviders
{
    public partial class PostgreSqlDataProvider : BaseDataProvider, IZarayeDataProvider
    {
        #region Fields
        ...
        private string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : BaseEntity
        {
            ...
            if (descriptor is null)
                throw new ZarayeException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
            ...
            if (string.IsNullOrEmpty(columnName))
                throw new ZarayeException("A table's primary key does not have an identity constraint");
        ...
        public virtual string BuildConnectionString(IZarayeConnectionStringInfo zarayeConnectionString)
        {
            if (zarayeConnectionString is null)
                throw new ArgumentNullException(nameof(zarayeConnectionString));

            if (zarayeConnectionString.IntegratedSecurity)
                throw new ZarayeException("Data provider supports connection only with login and password");

            var builder = new NpgsqlConnectionStringBuilder
            {
                Host = zarayeConnectionString.ServerName,
                //Cast DatabaseName to lowercase to avoid case-sensitivity problems
                Database = zarayeConnectionString.DatabaseName.ToLowerInvariant(),
                Username = zarayeConnectionString.Username,
                Password = zarayeConnectionString.Password,
            };
