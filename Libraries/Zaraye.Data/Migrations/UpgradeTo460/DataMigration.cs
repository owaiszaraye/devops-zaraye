...
{
    [ZarayeMigration("2022-07-20 00:00:01", "4.60.0", UpdateMigrationType.Data, MigrationProcessType.Update)]
    public class DataMigration : Migration
    {
        private readonly IZarayeDataProvider _dataProvider;

        public DataMigration(IZarayeDataProvider dataProvider)
        {
        ...
        public override void Up()
        {
            ...
            var customerRole = _dataProvider.GetTable<CustomerRole>().FirstOrDefault(cr => cr.SystemName == ZarayeCustomerDefaults.RegisteredRoleName);
            ...
            //#5809
            if (!_dataProvider.GetTable<ScheduleTask>().Any(st => string.Compare(st.Type, "Zaraye.Services.Gdpr.DeleteInactiveCustomersTask, Zaraye.Services", StringComparison.InvariantCultureIgnoreCase) == 0))
            {
                var manageConnectionStringPermission = _dataProvider.InsertEntity(
                    new ScheduleTask
                    {
                        ...
                        //24 hours
                        Seconds = 86400,
                        Type = "Zaraye.Services.Gdpr.DeleteInactiveCustomersTask, Zaraye.Services",
                        Enabled = false,
                        StopOnError = false
                    }
                );
            ...
            if (!_dataProvider.GetTable<PermissionRecord>().Any(pr => string.Compare(pr.SystemName, "EnableMultiFactorAuthentication", StringComparison.InvariantCultureIgnoreCase) == 0))
            {
                ...
                if (!forceMultifactorAuthentication)
                    customerRoles = customerRoles.Where(cr => cr.SystemName == ZarayeCustomerDefaults.AdministratorsRoleName || cr.SystemName == ZarayeCustomerDefaults.RegisteredRoleName);
            ...
            if (!_dataProvider.GetTable<ScheduleTask>().Any(st => string.Compare(st.Type, "Zaraye.Services.Common.ResetLicenseCheckTask, Zaraye.Services", StringComparison.InvariantCultureIgnoreCase) == 0))
            {
                _dataProvider.InsertEntity(new ScheduleTask
                {
                    Name = "ResetLicenseCheckTask",
                    Seconds = 2073600,
                    Type = "Zaraye.Services.Common.ResetLicenseCheckTask, Zaraye.Services",
                    Enabled = true,
