...
namespace Zaraye.Services.Localization
{
    ...
    public partial class LocalizationService : ILocalizationService
    {
        ...
        protected virtual async Task<IDictionary<string, string>> UpdateLocaleResourceAsync(IDictionary<string, string> resources, int? languageId = null, bool clearCache = true)
        {
            ...
            //clear cache
            if (clearCache)
                await _staticCacheManager.RemoveByPrefixAsync(ZarayeEntityCacheDefaults<LocaleStringResource>.Prefix);
        ...
        protected virtual IDictionary<string, string> UpdateLocaleResource(IDictionary<string, string> resources, int? languageId = null, bool clearCache = true)
        {
            ...
            //clear cache
            if (clearCache)
                _staticCacheManager.RemoveByPrefix(ZarayeEntityCacheDefaults<LocaleStringResource>.Prefix);
        ...
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeLocalizationDefaults.LocaleStringResourcesAllCacheKey, languageId);
            ...
            if (!loadPublicLocales.HasValue || allLocales != null)
            {
                ...
                await _staticCacheManager.RemoveAsync(ZarayeLocalizationDefaults.LocaleStringResourcesAllPublicCacheKey, languageId);
                await _staticCacheManager.RemoveAsync(ZarayeLocalizationDefaults.LocaleStringResourcesAllAdminCacheKey, languageId);
            ...
            key = _staticCacheManager.PrepareKeyForDefaultCache(loadPublicLocales.Value
                ? ZarayeLocalizationDefaults.LocaleStringResourcesAllPublicCacheKey
                : ZarayeLocalizationDefaults.LocaleStringResourcesAllAdminCacheKey,
                languageId);
            ...
            return await _staticCacheManager.GetAsync(key, () =>
            {
                ...
                var query = from l in _lsrRepository.Table
                            ...
                            where l.LanguageId == languageId
                            select l;
                query = loadPublicLocales.Value ? query.Where(r => !r.ResourceName.StartsWith(ZarayeLocalizationDefaults.AdminLocaleStringResourcesPrefix)) : query.Where(r => r.ResourceName.StartsWith(ZarayeLocalizationDefaults.AdminLocaleStringResourcesPrefix));
            ...
            bool logIfNotFound = true, string defaultValue = "", bool returnEmptyIfNotFound = false)
        {
            ...
            if (_localizationSettings.LoadAllLocaleRecordsOnStartup)
            {
                ...
                var resources = await GetAllResourceValuesAsync(languageId, !resourceKey.StartsWith(ZarayeLocalizationDefaults.AdminLocaleStringResourcesPrefix, StringComparison.InvariantCultureIgnoreCase));
            ...
            {
                //gradual loading
                var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeLocalizationDefaults.LocaleStringResourcesByNameCacheKey
        ...
        public virtual async Task<string> ExportResourcesToXmlAsync(Language language)
        {
            ...
            await xmlWriter.WriteAttributeStringAsync("SupportedVersion", ZarayeVersion.CURRENT_VERSION);
        ...
        public virtual async Task<string> GetLocalizedEnumAsync<TEnum>(TEnum enumValue, int? languageId = null) where TEnum : struct
        {
            ...
            //localized value
            var workingLanguage = await _workContext.GetWorkingLanguageAsync();
            var resourceName = $"{ZarayeLocalizationDefaults.EnumLocaleStringResourcesPrefix}{typeof(TEnum)}.{enumValue}";
        ...
        public virtual async Task<string> GetLocalizedPermissionNameAsync(PermissionRecord permissionRecord, int? languageId = null)
        {
            ...
            //localized value
            var workingLanguage = await _workContext.GetWorkingLanguageAsync();
            var resourceName = $"{ZarayeLocalizationDefaults.PermissionLocaleStringResourcesPrefix}{permissionRecord.SystemName}";
            var result = await GetResourceAsync(resourceName, languageId ?? workingLanguage.Id, false, string.Empty, true);
        ...
        public async Task<Dictionary<string, KeyValuePair<int, string>>> GetAllResourceValuesAsync(int languageId, string resourceName, bool? loadPublicLocales)
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeLocalizationDefaults.LocaleStringResourcesByNameCacheKey, resourceName,languageId);
            ...
            var rez = allLocales ?? await _staticCacheManager.GetAsync(key, () =>
            {
                ...
                var query = from l in _lsrRepository.Table
                            orderby l.ResourceName
                            where l.LanguageId == languageId && (l.ResourceName.Contains(resourceName) || l.ResourceName.Contains(resourceName.ToLowerInvariant()))
                            select l;
