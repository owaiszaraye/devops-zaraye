...
namespace Zaraye.Services.Messages
{
    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        private readonly IIndustryService _inustryService;
        ...
        public MessageTokenProvider(CatalogSettings catalogSettings,
            ...
            IIndustryService inustryService)
        {
            ...
            _inustryService = inustryService;
        }
        ...
        protected Dictionary<string, IEnumerable<string>> AllowedTokens
        {
            get
            {
                ...
                //Online lead
                _allowedTokens.Add(TokenGroupNames.OnlineLeadToRequest, new[]
                {
                "%OnlineLead.RequestId%",
                "%OnlineLead.Industry%",
                "%OnlineLead.Service%",
                "%OnlineLead.Brand%",
                "%OnlineLead.Quantity%",
                ...
                "%OnlineLead.Name%",
                "%OnlineLead.Email%",
                "%OnlineLead.ContactNumber%",
                "%OnlineLead.Country%",
                "%OnlineLead.City%",
                "%OnlineLead.Source%",
                "%OnlineLead.CreatedOn%"
        ...
        /// </returns>
        protected virtual async Task<string> ProductListToHtmlTableAsync(Order order, int languageId)
        {
            ...
            //we render checkout attributes and totals only for store owners
        ...
        /// <param name="languageId">Language identifier</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        //public virtual async Task AddOrderTokensAsync(IList<Token> tokens, Order order, int languageId)
        //{
        ...
        //    tokens.Add(new Token("Order.Product(s)", await ProductListToHtmlTableAsync(order, languageId), true));
        ...
        public virtual async Task AddCustomerTokensAsync(IList<Token> tokens, Customer customer)
        {
            ...
            var passwordRecoveryUrl  = await RouteUrlAsync(routeName: "PasswordRecoveryConfirm", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(customer, ZarayeCustomerDefaults.PasswordRecoveryTokenAttribute), guid = customer.CustomerGuid });
            var accountActivationUrl  = await RouteUrlAsync(routeName: "AccountActivation", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(customer, ZarayeCustomerDefaults.AccountActivationTokenAttribute), guid = customer.CustomerGuid });
            var emailRevalidationUrl  = await RouteUrlAsync(routeName: "EmailRevalidation", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(customer, ZarayeCustomerDefaults.EmailRevalidationTokenAttribute), guid = customer.CustomerGuid });
        ...
        public virtual IEnumerable<string> GetTokenGroups(MessageTemplate messageTemplate)
        {
            ...
            return messageTemplate.Name switch
            {
                ...
                MessageTemplateSystemNames.OnlineLeadConvertLeadToRequest => new[] { TokenGroupNames.OnlineLeadToRequest },
        ...
        public virtual async Task AddOnlineLeadTokensAsync(IList<Token> tokens, OnlineLead onlineLead)
        {
            tokens.Add(new Token("OnlineLead.Service", onlineLead.Service));
            tokens.Add(new Token("OnlineLead.Quantity", onlineLead.Quantity));
            tokens.Add(new Token("OnlineLead.Unit", onlineLead.Unit));
            tokens.Add(new Token("OnlineLead.Name", onlineLead.Name));
            tokens.Add(new Token("OnlineLead.ContactNumber", onlineLead.ContactNumber));
            tokens.Add(new Token("OnlineLead.CreatedOn", onlineLead.CreatedOnUtc.ToString()));

            //event notification
            await _eventPublisher.EntityTokensAddedAsync(onlineLead, tokens);
        }

        public virtual async Task AddOnlineLeadRequestTokensAsync(IList<Token> tokens, Customer buyer, Request Request)
        {
            var industry = await _inustryService.GetIndustryByIdAsync(Request.IndustryId);
            var product = await _productService.GetProductByIdAsync(Request.ProductId);
            var QtyType = (await _measureService.GetMeasureWeightByIdAsync(product.UnitId))?.Name ?? string.Empty;
            var CountryName = (await _countryService.GetCountryByIdAsync(Request.CountryId))?.Name;
            var CityName = (await _stateProvinceService.GetStateProvinceByIdAsync(Request.CityId))?.Name;
            var Brand = await _localizationService.GetLocalizedAsync((await _brandService.GetManufacturerByIdAsync(Request.BrandId)), x => x.Name);

            tokens.Add(new Token("OnlineLead.RequestId", Request.CustomRequestNumber));
            tokens.Add(new Token("OnlineLead.Industry", industry.Name));
            tokens.Add(new Token("OnlineLead.Service", product.Name));
            tokens.Add(new Token("OnlineLead.Brand", Brand));
            tokens.Add(new Token("OnlineLead.Quantity", Request.Quantity));
            tokens.Add(new Token("OnlineLead.Unit", QtyType));
            tokens.Add(new Token("OnlineLead.Name", buyer.FullName));
            tokens.Add(new Token("OnlineLead.Email", buyer.Email));
            tokens.Add(new Token("OnlineLead.ContactNumber", buyer.Username));
            tokens.Add(new Token("OnlineLead.Country", CountryName));
            tokens.Add(new Token("OnlineLead.City", CityName));
            tokens.Add(new Token("OnlineLead.Source", Request.Source));
            tokens.Add(new Token("OnlineLead.CreatedOn", Request.CreatedOnUtc.ToString()));
            //event notification
            await _eventPublisher.EntityTokensAddedAsync(Request, tokens);
        }

        public virtual async Task AddFeedbackTokensAsync(IList<Token> tokens, int customerId, int feedbackId)
        {
            if (customerId <= 0)
                throw new ArgumentOutOfRangeException(nameof(customerId));

            var customer = await _customerService.GetCustomerByIdAsync(customerId);
            var feedback = await _appFeedBackService.GetFeedBackByIdAsync(feedbackId);

            await AddFeedbackTokensAsync(tokens, customer, feedback);
        }

        public virtual async Task AddFeedbackTokensAsync(IList<Token> tokens, Customer customer, AppFeedBack appFeedBack)
        {
            tokens.Add(new Token("Feedback.Feedback", appFeedBack.FeedBack));
            tokens.Add(new Token("Feedback.Rating", appFeedBack.Rating));

            //event notification
            await _eventPublisher.EntityTokensAddedAsync(customer, tokens);
        }

        public virtual async Task AddBuyerTokensAsync(IList<Token> tokens, Customer buyer)
        {
            tokens.Add(new Token("Buyer.Email", buyer.Email));
            tokens.Add(new Token("Buyer.Username", buyer.Username));
            tokens.Add(new Token("Buyer.FullName", buyer.FullName));
            tokens.Add(new Token("Buyer.FirstName", buyer.FirstName));
            tokens.Add(new Token("Buyer.LastName", buyer.LastName));
            tokens.Add(new Token("Buyer.VatNumber", await _genericAttributeService.GetAttributeAsync<string>(buyer, ZarayeCustomerDefaults.VatNumberAttribute)));
            tokens.Add(new Token("Buyer.VatNumberStatus", ((VatNumberStatus)await _genericAttributeService.GetAttributeAsync<int>(buyer, ZarayeCustomerDefaults.VatNumberStatusIdAttribute)).ToString()));

            var customAttributesXml = await _genericAttributeService.GetAttributeAsync<string>(buyer, ZarayeCustomerDefaults.CustomCustomerAttributes);
            tokens.Add(new Token("Buyer.CustomAttributes", await _customerAttributeFormatter.FormatAttributesAsync(customAttributesXml), true));

            //note: we do not use SEO friendly URLS for these links because we can get errors caused by having .(dot) in the URL (from the email address)
            var passwordRecoveryUrl = await RouteUrlAsync(routeName: "PasswordRecoveryConfirm", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(buyer, ZarayeCustomerDefaults.PasswordRecoveryTokenAttribute), guid = buyer.CustomerGuid });
            var accountActivationUrl = await RouteUrlAsync(routeName: "AccountActivation", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(buyer, ZarayeCustomerDefaults.AccountActivationTokenAttribute), guid = buyer.CustomerGuid });
            var emailRevalidationUrl = await RouteUrlAsync(routeName: "EmailRevalidation", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(buyer, ZarayeCustomerDefaults.EmailRevalidationTokenAttribute), guid = buyer.CustomerGuid });
            var wishlistUrl = await RouteUrlAsync(routeName: "Wishlist", routeValues: new { customerGuid = buyer.CustomerGuid });
            tokens.Add(new Token("Buyer.PasswordRecoveryURL", passwordRecoveryUrl, true));
            tokens.Add(new Token("Buyer.AccountActivationURL", accountActivationUrl, true));
            tokens.Add(new Token("Buyer.EmailRevalidationURL", emailRevalidationUrl, true));

            //event notification
            await _eventPublisher.EntityTokensAddedAsync(buyer, tokens);
        }

        public virtual async Task AddSupplierTokensAsync(IList<Token> tokens, int supplierId)
        {
            if (supplierId <= 0)
            ...
            await AddSupplierTokensAsync(tokens, supplier);
        }

        public virtual async Task AddSupplierTokensAsync(IList<Token> tokens, Customer supplier)
        {
            tokens.Add(new Token("Supplier.Email", supplier.Email));
            tokens.Add(new Token("Supplier.Username", supplier.Username));
            tokens.Add(new Token("Supplier.FullName", supplier.FullName));
            tokens.Add(new Token("Supplier.FirstName", supplier.FirstName));
            tokens.Add(new Token("Supplier.LastName", supplier.LastName));
            tokens.Add(new Token("Supplier.VatNumber", await _genericAttributeService.GetAttributeAsync<string>(supplier, ZarayeCustomerDefaults.VatNumberAttribute)));
            tokens.Add(new Token("Supplier.VatNumberStatus", ((VatNumberStatus)await _genericAttributeService.GetAttributeAsync<int>(supplier, ZarayeCustomerDefaults.VatNumberStatusIdAttribute)).ToString()));

            var customAttributesXml = await _genericAttributeService.GetAttributeAsync<string>(supplier, ZarayeCustomerDefaults.CustomCustomerAttributes);
            tokens.Add(new Token("Supplier.CustomAttributes", await _customerAttributeFormatter.FormatAttributesAsync(customAttributesXml), true));

            //note: we do not use SEO friendly URLS for these links because we can get errors caused by having .(dot) in the URL (from the email address)
            var passwordRecoveryUrl = await RouteUrlAsync(routeName: "PasswordRecoveryConfirm", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(supplier, ZarayeCustomerDefaults.PasswordRecoveryTokenAttribute), guid = supplier.CustomerGuid });
            var accountActivationUrl = await RouteUrlAsync(routeName: "AccountActivation", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(supplier, ZarayeCustomerDefaults.AccountActivationTokenAttribute), guid = supplier.CustomerGuid });
            var emailRevalidationUrl = await RouteUrlAsync(routeName: "EmailRevalidation", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(supplier, ZarayeCustomerDefaults.EmailRevalidationTokenAttribute), guid = supplier.CustomerGuid });
            var wishlistUrl = await RouteUrlAsync(routeName: "Wishlist", routeValues: new { customerGuid = supplier.CustomerGuid });
            tokens.Add(new Token("Supplier.PasswordRecoveryURL", passwordRecoveryUrl, true));
            tokens.Add(new Token("Supplier.AccountActivationURL", accountActivationUrl, true));
            tokens.Add(new Token("Supplier.EmailRevalidationURL", emailRevalidationUrl, true));

            //event notification
            await _eventPublisher.EntityTokensAddedAsync(supplier, tokens);
        }
