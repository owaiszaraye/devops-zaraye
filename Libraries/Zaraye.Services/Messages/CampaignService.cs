...
namespace Zaraye.Services.Messages
{
    ...
    public partial class CampaignService : ICampaignService
    {
        ...
        private readonly IRepository<CampaignEmail> _campaignEmailRepository;
        ...
        public CampaignService(ICustomerService customerService,
            ...
            IRepository<CampaignEmail> campaignEmailRepository)
        {
            ...
            _campaignEmailRepository = campaignEmailRepository;
        }
        ...
        #region Campaign Emails

        public virtual async Task InsertCampaignEmailAsync(CampaignEmail campaignEmail)
        {
            await _campaignEmailRepository.InsertAsync(campaignEmail);
        }

        public virtual async Task UpdateCampaignEmailAsync(CampaignEmail campaignEmail)
        {
            await _campaignEmailRepository.UpdateAsync(campaignEmail);
        }

        public virtual async Task DeleteCampaignEmailAsync(CampaignEmail campaignEmail)
        {
            await _campaignEmailRepository.DeleteAsync(campaignEmail);
        }

        public virtual async Task<CampaignEmail> GetCampaignEmailByIdAsync(int id)
        {
            return await _campaignEmailRepository.GetByIdAsync(id, cache => default);
        }

        public virtual async Task<IList<CampaignEmail>> GetAllCampaignEmailAsync(int campaignId = 0, bool showOnlyActive = false)
        {
            var campaigns = await _campaignEmailRepository.GetAllAsync(query =>
            {
                if (campaignId > 0)
        ...
        public virtual async Task<CampaignEmail> GetCampaignByEmailAsync(int campaignId, string email)
        {
            var campaign = await _campaignEmailRepository.GetAllAsync(query =>
            {
                query = query.Where(c => c.CampaignId == campaignId && c.Email == email);
                return query;
            });
