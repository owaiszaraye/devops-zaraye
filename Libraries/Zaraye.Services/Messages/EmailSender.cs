...
namespace Zaraye.Services.Messages
{
    ...
    public partial class EmailSender : IEmailSender
    {
        ...
        public EmailSender(IDownloadService downloadService, IZarayeFileProvider fileProvider, ISmtpBuilder smtpBuilder)
        {
            ...
            int attachedDownloadId = 0, IDictionary<string, string> headers = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(attachmentFilePath) && attachmentFilePath.Contains("~"))
            {
                attachmentFilePath = attachmentFilePath.TrimEnd(';');
                Dictionary<string, string> attachmentFiles = new Dictionary<string, string>();
                var multipleAttachments = attachmentFilePath.Split(";");
                foreach (var attachment in multipleAttachments)
                {
                    var files = attachment.Split("~");
                    attachmentFiles.Add(files[0], files[1]);
                }

                foreach (var file in attachmentFiles)
                {
                    multipart.Add(await CreateMimeAttachmentAsync(file.Key, file.Value));
                }
            }
            //create the file attachment for this e-mail message
            else if (!string.IsNullOrEmpty(attachmentFilePath) && _fileProvider.FileExists(attachmentFilePath))
            {
