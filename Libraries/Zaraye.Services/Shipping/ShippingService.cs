...
namespace Zaraye.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        public virtual async Task<IList<ShippingMethod>> GetAllShippingMethodsAsync(int? filterByCountryId = null)
        {
            ...
            { 
                return await _shippingMethodRepository.GetAllAsync(query =>
                {
                    ...
                    return query2;
                }, cache => cache.PrepareKeyForDefaultCache(ZarayeShippingDefaults.ShippingMethodsAllCacheKey, filterByCountryId));
            }
        ...
        {
            var warehouses = from wh in _warehouseRepository.Table
                             orderby wh.Name
                             where !wh.SupplierId.HasValue
                             select wh;

            if (!string.IsNullOrEmpty(name)) 
                warehouses = warehouses.Where(wh => wh.Name.Contains(name));
        ...
        public virtual async Task<Warehouse> GetVirtualWarehouseBySupplierAsync(int supplierId)
        {
            //no address specified. return any
            if (supplierId <= 0)
                return null;

            var warehouse = await _warehouseRepository.GetAllAsync(query =>
            {
                return from wh in query
                       where wh.SupplierId == supplierId    
                       orderby wh.Name
                       select wh;
            });
