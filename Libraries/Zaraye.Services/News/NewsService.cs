...
namespace Zaraye.Services.News
{
    ...
    public partial class NewsService : INewsService
    {
        ...
        public virtual async Task<IPagedList<NewsItem>> GetAllNewsAsync(int languageId = 0, int storeId = 0,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false, string title = null, bool showAppNews = false)
        {
            var news = await _newsItemRepository.GetAllPagedAsync(async query =>
            {
                ...
                if (showAppNews)
                    query = query.Where(n => n.IsAppNews);
        ...
        public virtual async Task<int> GetNewsCommentsCountAsync(NewsItem newsItem, int storeId = 0, bool? isApproved = null)
        {
            ...
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeNewsDefaults.NewsCommentsNumberCacheKey, newsItem, storeId, isApproved);
