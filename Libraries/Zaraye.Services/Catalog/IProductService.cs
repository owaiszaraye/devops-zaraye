...
namespace Zaraye.Services.Catalog
{
    ...
    public partial interface IProductService
    {
        ...
        /// The task result contains the products
        /// </returns>
        Task<IPagedList<Product>> GetLowStockProductsAsync(bool? loadPublishedOnly = true,
        ...
        /// </returns>
        Task<IPagedList<ProductAttributeCombination>> GetLowStockProductCombinationsAsync(bool? loadPublishedOnly = true,
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false);
        ...
        Task<IList<Product>> GetAllProductsByCategoryIdAsync(int categoryId, bool? isAppPublished = null);
        Task<IList<Product>> GetAllProductsByCategoryIdsAsync(int[] categoryIds);

        //Task<IPagedList<SellerProduct>> GetAllSellerByProductIdAsync(int productId,
        //  int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false);

        Task<IList<Product>> GetAllProductsByIndustryIdsAsync(int[] industryIds);

        Task<IPagedList<Product>> GetProductsByIndustryCategoryAndBrandIdsAsync(
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            ...
            bool? overridePublished = null,
            bool? isAppPublished = null);
        Task<IList<Product>> GetAllBestSellingProductsAsync(int pageIndex = 0, int pageSize = int.MaxValue);

        Task<IList<Product>> GetAllProductsByIndustryIdAsync(int industryId);

        Task<IList<Product>> GetAllBestSellingProductsByIndustryIdAsync(int industryId);
        ...
        /// The task result contains the ier price
        /// </returns>
        Task<TierPrice> GetPreferredTierPriceAsync(Product product, Customer customer, Store store, decimal quantity);
        ...
        Task<IPagedList<MspInventoryMapping>> GetMspInventoryMappingsAsync(int inventoryId,
            int pageIndex = 0, int pageSize = int.MaxValue);
        #endregion

        #region Supplier Product Mapping

        Task<SupplierProduct> GetSupplierProductByIdAsync(int supplierProductId);
        ...
        Task DeleteSupplierProductAsync(SupplierProduct supplierProduct);

        Task<List<SupplierProduct>> GetAllSupplierProductsAsync();

        Task<IPagedList<SupplierProduct>> GetSupplierProductsByProductIdAsync(int productId,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false);

        Task<IPagedList<SupplierProduct>> GetSupplierProductsBySupplierIdAsync(int supplierId,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false);

        Task<IPagedList<SupplierProduct>> GetSuppliersByProductIdAsync(int productId,
            int pageIndex = 0, int pageSize = int.MaxValue);

        Task<IPagedList<Product>> GetAllProductsBySupplierIdAsync(int supplierId,
            bool showHidden = false, int pageIndex = 0, int pageSize = int.MaxValue);

        Task<IList<Category>> GetAllCategoriesBySupplierIdAsync(int supplierId, bool showHidden = false);

        Task<SupplierProduct> GetSupplierAppliedToProductAsync(int productId, int supplierId);

        SupplierProduct FindSupplierProduct(IList<SupplierProduct> source, int productId, int supplierId);

        Task<IPagedList<SupplierProduct>> GetAllSupplierByProductIdAsync(int productId,
           int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false);

        Task<IList<SupplierProduct>> GetAllSupplierByProductCacheIdAsync(int productId);

        #endregion

        #region Rate 

        Task InsertRateAsync(Rate rate, int industryId, int categoryId, int productId, string productAttributeXml, int brandId);

        Task InsertRateAsync(Rate rate);

        Task<Rate> GetLastRateByGroupIdAsync(int groupId);

        Task<IPagedList<Rate>> GetAllRatesByGroupIdAsync(int rateGroupId = 0, int pageIndex = 0, int pageSize = int.MaxValue);

        #endregion

        #region Rate Group
        Task<RateGroup> GetRateGroupByIdAsync(int id);
        Task<RateGroup> GetRateGroupAsync(int industryId = 0, int categoryId = 0, int productId = 0, string productAttributeXml = null, int brandId = 0);
        Task<List<RateGroup>> GetRateGroupListAsync(int industryId = 0, int categoryId = 0, int productId = 0, string productAttributeXml = null, int brandId = 0);
        Task<IList<RateGroup>> GetAllGroupRateListAsync();
        Task<IList<RateGroupList>> GetAllRateListAsync(int industryId = 0, int categoryId = 0, int userId = 0);
        Task InsertRateGroupAsync(RateGroup rateGroup);
        #endregion
    }
}