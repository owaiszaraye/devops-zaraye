...
namespace Zaraye.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        protected readonly IRepository<Rate> _rateRepository;
        protected readonly IRepository<RateGroup> _rateGroupRepository;
        protected readonly IStaticCacheManager _staticCacheManager;
        protected readonly IStoreMappingService _storeMappingService;
        protected readonly IStoreService _storeService;
        protected readonly IWorkContext _workContext;
        protected readonly LocalizationSettings _localizationSettings;
        protected readonly IRepository<MspInventoryMapping> _mspInventoryMappingRepository;
        protected readonly IRepository<SupplierProduct> _supplierProductRepository;
        protected readonly IZarayeDataProvider _zarayeDataProvider;
        protected readonly IRepository<Industry> _industryRepository;
        ...
        public ProductService(CatalogSettings catalogSettings,
            ...
            IRepository<RateGroup> rateGroupRepository,
            IStaticCacheManager staticCacheManager,
            IStoreService storeService,
            IStoreMappingService storeMappingService,
            IWorkContext workContext,
            LocalizationSettings localizationSettings,
            IRepository<MspInventoryMapping> mspInventoryMappingRepository,
            ...
            IZarayeDataProvider zarayeDataProvider)
        {
            ...
            _rateGroupRepository = rateGroupRepository;
            _staticCacheManager = staticCacheManager;
            _storeMappingService = storeMappingService;
            _storeService = storeService;
            _workContext = workContext;
            _localizationSettings = localizationSettings;
            _mspInventoryMappingRepository = mspInventoryMappingRepository;
        ...
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.ProductsHomepageCacheKey);

            var productlist = await _staticCacheManager.GetAsync(key, async () =>
            {
                var products = await _productRepository.GetAllAsync(query =>
                {
                    return from p in query
                           ...
                           where p.Published &&
                                 ...
                                 p.ShowOnHomepage
                           select p;
                }, cache => cache.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.ProductsHomepageCacheKey));
        ...
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.ProductByIdCacheKey, $"ProductId{productId}");

            var products = await _staticCacheManager.GetAsync(key, async () =>
            {
        ...
        public virtual async Task<IList<Product>> GetCategoryFeaturedProductsAsync(int categoryId, int storeId = 0)
        {
            ...
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoryFeaturedProductsIdsKey, categoryId, customerRoleIds, storeId);
        ...
        public virtual async Task<IList<Product>> GetManufacturerFeaturedProductsAsync(int manufacturerId, int storeId = 0)
        {
            ...
            var customer = await _workContext.GetCurrentCustomerAsync();
            var customerRoleIds = await _customerService.GetCustomerRoleIdsAsync(customer);
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.ManufacturerFeaturedProductIdsKey, manufacturerId, customerRoleIds, storeId);
        ...
        public virtual async Task<int> GetNumberOfProductsInCategoryAsync(IList<int> categoryIds = null, int storeId = 0)
        {
            ...
            var cacheKey = _staticCacheManager
                .PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoryProductsNumberCacheKey, customerRoleIds, storeId, categoryIds);
        ...
        public virtual async Task<IPagedList<Product>> SearchProductsAsync(
            ...
            IList<int> filteredAttributeMappingIds = null,
            bool? isAppPublished = null)
        {
            ...
            if (isAppPublished.HasValue && isAppPublished.Value)
                productsQuery = productsQuery.Where(m => m.AppPublished);
            ...
            if (filteredAttributeMappingIds is not null)
            {
                if (filteredAttributeMappingIds.Contains(0))
                    filteredAttributeMappingIds.Remove(0);

                if (filteredAttributeMappingIds.Any())
                {
                    var productAttributeQuery =
                        from pam in _productAttributeMappingRepository.Table
                        where filteredAttributeMappingIds.Contains(pam.Id)
                        select pam;

                    productsQuery =
                        from p in productsQuery
                        join pm in productAttributeQuery on p.Id equals pm.ProductId
                        select p;
                }
            }
            ...
            return await productsQuery.OrderBy(x => x.DisplayOrder).ToPagedListAsync(pageIndex, pageSize);
        }
        ...
        /// The task result contains the products
        /// </returns>
        public virtual async Task<IPagedList<Product>> GetLowStockProductsAsync(bool? loadPublishedOnly = true,
        ...
        /// </returns>
        public virtual async Task<IPagedList<ProductAttributeCombination>> GetLowStockProductCombinationsAsync(bool? loadPublishedOnly = true,
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
        ...
        public virtual async Task<IList<Product>> GetAllProductsByCategoryIdAsync(int categoryId, bool? isAppPublished = null)
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.ProductCategoryIdCacheKey, $"categoryId{categoryId}");

            var productsbyCategory = await _staticCacheManager.GetAsync(key, async () =>
            {
                var products = _productRepository.Table;
                products = from p in products
                           join pcm in _productCategoryRepository.Table on p.Id equals pcm.ProductId
                           where pcm.CategoryId == categoryId && !p.Deleted
                           orderby pcm.DisplayOrder
                           select p;

                if (isAppPublished.HasValue && isAppPublished.Value == true)
                    products = products.Where(m => m.Published || m.AppPublished);
            ...
            //        products = products.Where(m => m.Published || m.AppPublished);
            //    else
            //        products = products.Where(m => m.Published);
            //}
            //else if (overridePublished.HasValue)
            //    products = products.Where(c => c.Published == overridePublished.Value);

            //if (isAppPublished.HasValue && isAppPublished.Value)
            //    products = products.Where(m => m.AppPublished);
        ...
        public virtual async Task<IList<Product>> GetAllProductsByCategoryIdsAsync(int[] categoryIds)
        {
            var products = _productRepository.Table;
            products = from p in products
                       join pcm in _productCategoryRepository.Table on p.Id equals pcm.ProductId
                       where categoryIds.Contains(pcm.CategoryId) && p.AppPublished && !p.Deleted
                       orderby pcm.DisplayOrder
                       select p;
        ...
        public virtual async Task<IList<Product>> GetAllProductsByIndustryIdsAsync(int[] industryIds)
        {
            var products = _productRepository.Table;
            if (industryIds.Any())
            {
                products = from p in products where industryIds.Contains(p.IndustryId) && p.AppPublished && !p.Deleted orderby p.DisplayOrder select p;
            }
            else
            {
                products = from p in products where p.AppPublished && !p.Deleted select p;
            }

            return await products.ToListAsync();
        }

        public virtual async Task<IList<Product>> GetAllProductsByIndustryIdAsync(int industryId)
        {
            var products = _productRepository.Table;
            if (industryId > 0)
            {
                products = (from p in products
                            join cat in _categoryRepository.Table on p.IndustryId equals cat.IndustryId
                            join pcm in _productCategoryRepository.Table on p.Id equals pcm.ProductId
                            where p.IndustryId == industryId && p.Published && !p.Deleted && !cat.Deleted && cat.Published
                            && pcm.CategoryId == cat.Id && pcm.ProductId == p.Id
            ...
            else
            {
                products = (from p in products
                            join cat in _categoryRepository.Table on p.IndustryId equals cat.IndustryId
                            join pcm in _productCategoryRepository.Table on p.Id equals pcm.ProductId
                            where p.Published && !p.Deleted && !cat.Deleted && cat.Published
                            && pcm.CategoryId == cat.Id && pcm.ProductId == p.Id
                            orderby p.DisplayOrder
                            select p).Distinct();
            }
        ...
        public virtual async Task<IPagedList<Product>> GetProductsByIndustryCategoryAndBrandIdsAsync(
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            IList<int> industryIds = null,
            IList<int> categoryIds = null,
            IList<int> manufacturerIds = null,
            int languageId = 0,
            ProductSortingEnum orderBy = ProductSortingEnum.Position,
            ...
            bool? isAppPublished = null)
        {
            ...
            if (industryIds.Any())
            {
                productsQuery = (from p in productsQuery
                                 join cat in _categoryRepository.Table on p.IndustryId equals cat.IndustryId
                                 join pcm in _productCategoryRepository.Table on p.Id equals pcm.ProductId
                                 where industryIds.Contains(p.IndustryId) && p.Published && !p.Deleted && p.IndustryId > 0 && !cat.Deleted && cat.Published
                                 && pcm.CategoryId == cat.Id && pcm.ProductId == p.Id
                                 orderby p.DisplayOrder
                                 select p).Distinct();
            }
            else
            {
                productsQuery = (from p in productsQuery
                                 join cat in _categoryRepository.Table on p.IndustryId equals cat.IndustryId
                                 join pcm in _productCategoryRepository.Table on p.Id equals pcm.ProductId
                                 where p.Published && !p.Deleted && p.IndustryId > 0 && !cat.Deleted && cat.Published && pcm.CategoryId == cat.Id && pcm.ProductId == p.Id
                                 orderby p.DisplayOrder
                                 select p).Distinct();
            }
            ...
            if (categoryIds.Any())
            {
                ...
                productsQuery = (from p in productsQuery
                                 join pcm in _productCategoryRepository.Table on p.Id equals pcm.ProductId
                                 where categoryIds.Contains(pcm.CategoryId) && p.Published && !p.Deleted
                                 orderby p.DisplayOrder
                                 select p).Distinct();
            ...
            {
                productsQuery = (from p in productsQuery
                                 join pm in _productManufacturerRepository.Table on p.Id equals pm.ManufacturerId
                                 where manufacturerIds.Contains(pm.ManufacturerId) && p.Published && !p.Deleted
                                 orderby p.DisplayOrder
                                 select p).Distinct();
        ...
        //public virtual async Task<IPagedList<SellerProduct>> GetAllSellerByProductIdAsync(int productId,
        //  int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        //{
        //    if (productId == 0)
        //        return new PagedList<SellerProduct>(new List<SellerProduct>(), pageIndex, pageSize);

        //    var query = from sp in _sellerProductRepository.Table
        //                join p in _productRepository.Table on sp.ProductId equals p.Id
        //                where p.Id == productId && !p.Deleted && (showHidden || p.Published)
        //                orderby sp.DisplayOrder, sp.Id
        //                select sp;

        //    return await query.ToPagedListAsync(pageIndex, pageSize);
        //}
        public async Task<IList<Product>> GetAllBestSellingProductsAsync(int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.BestSellingProductsCacheKey, $"PageSizePrefix{pageSize}", $"PageIndexPrefix{pageIndex}");
            var bestProducts = await _staticCacheManager.GetAsync(key, async () =>
            {
                var products = await (from p in _productRepository.Table
                                      where !p.Deleted && p.BestSellingProduct == true
                                      orderby p.Id
                                      select p).ToPagedListAsync(pageIndex, pageSize);
        ...
        public async Task<IList<Product>> GetAllBestSellingProductsByIndustryIdAsync(int industryId)
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.BestSellingProductsByIndustryIdCacheKey, $"industryId{industryId}");

            var bestProducts = await _staticCacheManager.GetAsync(key, async () =>
            {
                var products = await (from p in _productRepository.Table
                                      where p.IndustryId == industryId && !p.Deleted && p.BestSellingProduct == true
                                      orderby p.Id
                                      select p).ToPagedListAsync(0, int.MaxValue);
                if (products.Count <= 0 && products == null)
                    throw new ApplicationException("Record not found.");
                return products;
            });
        ...
        public virtual async Task<IList<RelatedProduct>> GetRelatedProductsByProductId1Async(int productId, bool showHidden = false)
        {
            ...
            var relatedProducts = await _staticCacheManager.GetAsync(_staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.RelatedProductsCacheKey, productId, showHidden), async () => await query.ToListAsync());
        ...
        public virtual async Task<IList<TierPrice>> GetTierPricesByProductAsync(int productId)
        {
            ...
            return await _staticCacheManager.GetAsync(_staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.TierPricesByProductCacheKey, productId), async () => await query.ToListAsync());
        }
        ...
        /// </returns>
        public virtual async Task<TierPrice> GetPreferredTierPriceAsync(Product product, Customer customer, Store store, decimal quantity)
        {
            if (product is null)
                throw new ArgumentNullException(nameof(product));
        ...
        public virtual async Task<IPagedList<ProductReview>> GetAllProductReviewsAsync(int customerId = 0, bool? approved = null,
            ...
            string message = null, int storeId = 0, int productId = 0, bool showHidden = false,
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
        ...
        public virtual async Task<SupplierProduct> GetSupplierProductByIdAsync(int supplierProductId)
        {
            return await _supplierProductRepository.GetByIdAsync(supplierProductId, cache => default);
        }

        public virtual async Task InsertSupplierProductAsync(SupplierProduct supplierProduct)
        {
            await _supplierProductRepository.InsertAsync(supplierProduct);
        }

        public virtual async Task UpdateSupplierProductAsync(SupplierProduct supplierProduct)
        {
            await _supplierProductRepository.UpdateAsync(supplierProduct);
        }

        public virtual async Task DeleteSupplierProductAsync(SupplierProduct supplierProduct)
        {
            await _supplierProductRepository.DeleteAsync(supplierProduct);
        }

        public virtual async Task<List<SupplierProduct>> GetAllSupplierProductsAsync()
        {
        ...
        public virtual async Task<IPagedList<SupplierProduct>> GetSupplierProductsByProductIdAsync(int productId,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            if (productId == 0)
                return new PagedList<SupplierProduct>(new List<SupplierProduct>(), pageIndex, pageSize);

            var query = from sp in _supplierProductRepository.Table
                        join p in _productRepository.Table on sp.ProductId equals p.Id
                        where sp.ProductId == productId && !p.Deleted && (showHidden || p.Published)
                        orderby sp.DisplayOrder, sp.Id
                        select sp;
        ...
        public virtual async Task<IPagedList<SupplierProduct>> GetSupplierProductsBySupplierIdAsync(int supplierId,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            if (supplierId == 0)
                return new PagedList<SupplierProduct>(new List<SupplierProduct>(), pageIndex, pageSize);
            ...
            var query = from sp in _supplierProductRepository.Table
                        join p in _productRepository.Table on sp.ProductId equals p.Id
                        where sp.SupplierId == supplierId && !p.Deleted && (showHidden || p.Published)
                        orderby sp.DisplayOrder, sp.Id
                        select sp;
        ...
        public virtual async Task<IPagedList<SupplierProduct>> GetSuppliersByProductIdAsync(int productId,
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            if (productId == 0)
                return new PagedList<SupplierProduct>(new List<SupplierProduct>(), pageIndex, pageSize);
        ...
        public virtual async Task<IPagedList<Product>> GetAllProductsBySupplierIdAsync(int supplierId,
            bool showHidden = false, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var products = _productRepository.Table;
            products = from product in products
                       join sp in _supplierProductRepository.Table on product.Id equals sp.ProductId
                       where sp.SupplierId == supplierId && (showHidden || sp.Published)
                       select product;

            if (!showHidden)
                products = products.Where(p => !p.Deleted);

            products = products.OrderBy(category => category.DisplayOrder).ThenBy(category => category.Id);
            return await products.ToPagedListAsync(pageIndex, pageSize);
        }

        public virtual async Task<IList<Category>> GetAllCategoriesBySupplierIdAsync(int supplierId, bool showHidden = false)
        {
            var categories = _categoryRepository.Table;
            categories = from category in categories
                         join pc in _productCategoryRepository.Table on category.Id equals pc.CategoryId
                         join sp in _supplierProductRepository.Table on pc.ProductId equals sp.ProductId
                         where sp.SupplierId == supplierId && (showHidden || sp.Published)
                         select category;

            if (!showHidden)
                categories = categories.Where(p => !p.Deleted);
        ...
        public virtual async Task<SupplierProduct> GetSupplierAppliedToProductAsync(int productId, int supplierId)
        {
            return await _supplierProductRepository.Table
                .FirstOrDefaultAsync(dcm => dcm.ProductId == productId && dcm.SupplierId == supplierId);
        }

        public virtual SupplierProduct FindSupplierProduct(IList<SupplierProduct> source, int productId, int supplierId)
        {
            foreach (var supplierProduct in source)
                if (supplierProduct.ProductId == productId && supplierProduct.SupplierId == supplierId)
                    return supplierProduct;

            return null;
        }

        public virtual async Task<IPagedList<SupplierProduct>> GetAllSupplierByProductIdAsync(int productId,
           int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            if (productId == 0)
                return new PagedList<SupplierProduct>(new List<SupplierProduct>(), pageIndex, pageSize);
            ...
            var query = from sp in _supplierProductRepository.Table
                        join p in _productRepository.Table on sp.ProductId equals p.Id
                        where p.Id == productId && !p.Deleted && (showHidden || p.Published || p.AppPublished)
                        orderby sp.DisplayOrder, sp.Id
                        select sp;
        ...
        public virtual async Task<IList<SupplierProduct>> GetAllSupplierByProductCacheIdAsync(int productId)
        {
            if (productId == 0)
                return new List<SupplierProduct>();

            //var key = _staticCacheManager.PrepareKeyForDefaultCache(NopCatalogDefaults.AllSuppliersByProductCacheKey, productId);
        ...
        public virtual async Task<IPagedList<MspInventoryMapping>> GetMspInventoryMappingsAsync(int inventoryId,
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _mspInventoryMappingRepository.Table.Where(historyEntry => historyEntry.InventoryId == inventoryId);
        ...
        public virtual async Task InsertRateAsync(Rate rate, int industryId, int categoryId, int productId, string productAttributeXml, int brandId)
        {
            //get Group
            var rateGroup = await GetRateGroupAsync(industryId: industryId, categoryId: categoryId, productId: productId, productAttributeXml, brandId);
            if (rateGroup is null)
            {
                rateGroup = new RateGroup
                {
                    ...
                    ProductId = productId,
                    ProductAttributeXml = productAttributeXml,
                    BrandId = brandId,
                };

                //Add Group
                await InsertRateGroupAsync(rateGroup);
        ...
        public virtual async Task InsertRateAsync(Rate rate)
        {
            await _rateRepository.InsertAsync(rate);
        }

        public virtual async Task<Rate> GetLastRateByGroupIdAsync(int groupId)
        {
            var query = from r in _rateRepository.Table
                        where r.RateGroupId == groupId
        ...
        public virtual async Task<RateGroup> GetRateGroupByIdAsync(int id)
        {
            return await _rateGroupRepository.GetByIdAsync(id, cache => default);
        }
        public virtual async Task<RateGroup> GetRateGroupAsync(int industryId = 0, int categoryId = 0, int productId = 0, string productAttributeXml = null, int brandId = 0)
        {
            return await (from r in _rateGroupRepository.Table
                          where r.IndustryId == industryId && r.CategoryId == categoryId
                          && r.ProductId == productId && r.BrandId == brandId &&
                          r.ProductAttributeXml == productAttributeXml
                          select r).FirstOrDefaultAsync();
        }
        public virtual async Task<List<RateGroup>> GetRateGroupListAsync(int industryId = 0, int categoryId = 0, int productId = 0, string productAttributeXml = null, int brandId = 0)
        {
            return await (from r in _rateGroupRepository.Table
                          where r.IndustryId == industryId && r.CategoryId == categoryId
                          && r.ProductId == productId && r.BrandId == brandId &&
        ...
        public virtual async Task<IList<RateGroup>> GetAllGroupRateListAsync()
        {
            var query = from r in _rateGroupRepository.Table
                        orderby r.CreatedOnUtc descending
                        select r;
        ...
        public virtual async Task<IList<RateGroupList>> GetAllRateListAsync(int industryId = 0, int categoryId = 0, int userId = 0)
        {
            var allRates = await _zarayeDataProvider.QueryProcAsync<RateGroupList>("get_rate_list",
            ...
            return await allRates.AsQueryable().ToListAsync();
        }
        public virtual async Task InsertRateGroupAsync(RateGroup rateGroup)
        {
            await _rateGroupRepository.InsertAsync(rateGroup);
        }

        public virtual async Task<IPagedList<Rate>> GetAllRatesByGroupIdAsync(int rateGroupId = 0, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            return await (from r in _rateRepository.Table
                          where r.RateGroupId == rateGroupId && r.Deleted == false
                          select r).ToPagedListAsync(pageIndex, pageSize);
        }
        public virtual async Task<IList<Product>> GetAllProductsAsync()
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.AllProductCacheKey);

            var productsbyCategory = await _staticCacheManager.GetAsync(key, async () =>
            {
                var products = _productRepository.Table;
                products = from p in products
                           where p.Published || p.AppPublished && !p.Deleted
                           orderby p.DisplayOrder
                           select p;
                return await products.ToListAsync();
            });
