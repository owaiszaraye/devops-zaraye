...
namespace Zaraye.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        private readonly IRepository<CategoryCategoryTagMapping> _categoryTagMappingRepository;
        ...
        public CategoryService(
            ...
            IRepository<CategoryCategoryTagMapping> categoryTagMappingRepository,
            IRepository<CategoryTag> categoryTagRepository)
        {
            ...
            _categoryTagMappingRepository = categoryTagMappingRepository;
        ...
        protected virtual async Task<IList<ProductCategory>> GetProductCategoriesByProductIdAsync(int productId, int storeId,
            bool showHidden = false, bool? isAppPublished = null)
        {
            if (productId == 0)
            ...
            return await _productCategoryRepository.GetAllAsync(async query =>
            {
                if (!showHidden)
                {
                    ...
                    if (isAppPublished.HasValue && isAppPublished.Value == true)
                        categoriesQuery = _categoryRepository.Table.Where(m => m.Published || m.AppPublished);
                    else
                        categoriesQuery = _categoryRepository.Table.Where(m => m.Published);
            ...
            }, cache => _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.ProductCategoriesByProductCacheKey,
        ...
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoriesAllCacheKey,
                storeId,
        ...
        /// The task result contains the categories
        /// </returns>
        public virtual async Task<IPagedList<Category>> GetAllCategoriesAsync(string categoryName, int storeId = 0, int industryId = 0,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false, bool? overridePublished = null)
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoriesAllCacheKey,
                storeId, await _customerService.GetCustomerRoleIdsAsync(await _workContext.GetCurrentCustomerAsync()),
                showHidden);
            ...
            bool showHidden = false)
        {
            ...
            }, cache => cache.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoriesByParentCategoryCacheKey,
                parentCategoryId, showHidden, customer, store));

            return categories;
        }

        public virtual async Task<IList<Category>> GetAllCategoriesByIndustryIdAsync(int industryId, bool? isAppPublished = null)
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoriesByIndustryIdCacheKey, $"IndustryId{industryId}");
            var store = await _storeContext.GetCurrentStoreAsync();
            var customer = await _workContext.GetCurrentCustomerAsync();
            ...
            {
                if (isAppPublished.HasValue && isAppPublished.Value == true)
                    query = query.Where(m => m.Published || m.AppPublished);
                else
                ...
                query = query.Where(c => !c.Deleted && c.IndustryId == industryId);

                return query.OrderBy(c => c.DisplayOrder).ThenBy(c => c.Id);
            }, cache => cache.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoriesByIndustryCacheKey,
                industryId, customer, store));

            return categories;
        }

        public virtual async Task<IPagedList<Category>> GetCategoriesByIndustryIdsAsync(
          int pageIndex = 0,
          ...
          bool? isPublished = null)
        {
            ...
            if (industryIds.Any())
            {
                categoryQuery = (from p in categoryQuery 
                                 where industryIds.Contains(p.IndustryId) && p.Published && !p.Deleted && p.IndustryId > 0 
            ...
            {
                categoryQuery = (from p in categoryQuery
                                 where p.Published && !p.Deleted && p.IndustryId > 0
                                 orderby p.DisplayOrder
                                 select p).Distinct();
            }

            return await categoryQuery.OrderBy(x => x.DisplayOrder).ToPagedListAsync(pageIndex, pageSize);
            ...
            var categories = await _categoryRepository.GetAllAsync(query =>
            {
                return from c in query
                       ...
                       where c.Published &&
                             ...
                             c.ShowOnHomepage
                       select c;
            }, cache => cache.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoriesHomepageCacheKey));

            if (showHidden)
                return categories;

            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoriesHomepageWithoutHiddenCacheKey,
        ...
        public virtual async Task<IList<int>> GetAppliedCategoryIdsAsync(Discount discount, Customer customer)
        {
            ...
            var store = await _storeContext.GetCurrentStoreAsync();
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeDiscountDefaults.CategoryIdsByDiscountCacheKey,
                discount,
        ...
        {
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoriesChildIdsCacheKey,
                parentCategoryId,
                await _customerService.GetCustomerRoleIdsAsync(await _workContext.GetCurrentCustomerAsync()),
                storeId,
                showHidden);
        ...
        /// The task result contains the product category mapping collection
        /// </returns>
        public virtual async Task<IList<ProductCategory>> GetProductCategoriesByProductIdAsync(int productId, bool showHidden = false, bool? isAppPublished = null)
        {
            var store = await _storeContext.GetCurrentStoreAsync();

            return await GetProductCategoriesByProductIdAsync(productId, store.Id, showHidden, isAppPublished);
        }
        ...
        public virtual async Task<IList<Category>> GetCategoryBreadCrumbAsync(Category category, IList<Category> allCategories = null, bool showHidden = false)
        {
            ...
            var breadcrumbCacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.CategoryBreadcrumbCacheKey,
                category,
                await _customerService.GetCustomerRoleIdsAsync(await _workContext.GetCurrentCustomerAsync()),
        ...
        public virtual async Task<IList<Category>> GetAllCategoriesDisplayedOnHomepageByIndustryIdAsync(int ParentCategoryId)
        {
            var query = from c in _categoryRepository.Table
                        orderby c.DisplayOrder, c.Id
                        where c.Published &&
                              !c.Deleted &&
                              c.IndustryId == ParentCategoryId &&
                              c.ShowOnHomepage
                        select c;
        ...
        public virtual async Task<IList<Category>> SearchCategoriesAsync(string keywords, bool searchCategoryTags = false, bool showHidden = false, bool? overridePublished = null)
        {
            var query = _categoryRepository.Table;
            ...
            {
                IQueryable<int> CategoriesByKeywords;

                CategoriesByKeywords =
                        from c in _categoryRepository.Table
                        where c.Name.Contains(keywords)
                        select c.Id;


                if (searchCategoryTags)
                {
                    CategoriesByKeywords = CategoriesByKeywords.Union(
                        from cctm in _categoryTagMappingRepository.Table
                        join ct in _categoryTagRepository.Table on cctm.CategoryTagId equals ct.Id
                        where ct.Name == keywords
                        select cctm.CategoryId
                    );
                ...
                query =
                    from p in query
                    from pbk in LinqToDB.LinqExtensions.InnerJoin(CategoriesByKeywords, pbk => pbk == p.Id)
                    select p;
            }
