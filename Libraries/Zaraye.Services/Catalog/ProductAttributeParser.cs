    ...
    public partial class ProductAttributeParser : BaseAttributeParser, IProductAttributeParser
    {
                    ...
                    case AttributeControlType.ImageSquares:
                        {
                            ...
                            if (!StringValues.IsNullOrEmpty(ctrlAttributes))
                            {
                                ...
                                if (selectedAttributeId > 0)
                                {
                                    ...
                                    var quantity = 1;
                                    var quantityStr = form[$"{ZarayeCatalogDefaults.ProductAttributePrefix}{attribute.Id}_{selectedAttributeId}_qty"];
                            ...
                            if (!StringValues.IsNullOrEmpty(ctrlAttributes))
                            {
                                    ...
                                    .Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
                                {
                                    ...
                                    if (selectedAttributeId > 0)
                                    {
                                        //get quantity entered by customer
                                        var quantity = 1;
                                        var quantityStr = form[$"{ZarayeCatalogDefaults.ProductAttributePrefix}{attribute.Id}_{item}_qty"];
                                        if (!StringValues.IsNullOrEmpty(quantityStr) &&
                                            (!int.TryParse(quantityStr, out quantity) || quantity < 1))
                                            errors.Add(await _localizationService.GetResourceAsync("Products.QuantityShouldBePositive"));
                    ...
                    case AttributeControlType.MultilineTextbox:
                        {
                            ...
                            if (!StringValues.IsNullOrEmpty(ctrlAttributes))
                            {
                                ...
                                if (attribute.AttributeControlType == AttributeControlType.NumericTextBox)
                                {
                                    var enteredNumericText = Convert.ToInt32(ctrlAttributes.ToString().Trim());
                                    if (attribute.ValidationMinLength.Value > enteredNumericText)
                                    {
                                        errors.Add(string.Format(await _localizationService.GetResourceAsync("ProductAttribute.NumericTextboxMinimumLength"), await _localizationService.GetLocalizedAsync(attribute, a => a.TextPrompt), attribute.ValidationMinLength.Value));
                                    }
                                }
                            }
                            //else
        ...
        public virtual decimal ParseEnteredQuantity(Product product, IFormCollection form)
        {
            ...
            foreach (var formKey in form.Keys)
                ...
                {
                    _ = decimal.TryParse(form[formKey], out quantity);
                    break;
                }
        ...
        public virtual int ParseSelectedBrand(Product product, IFormCollection form)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            if (form == null)
            ...
            var brandId = 0;
            foreach (var formKey in form.Keys)
                if (formKey.Equals($"product_manufacturer_{product.Id}", StringComparison.InvariantCultureIgnoreCase))
                {
                    int.TryParse(form[formKey], out brandId);
                    break;
                }

            return brandId;
        }
        public virtual string ParseSelectedOtherBrand(Product product, IFormCollection form)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            if (form == null)
            ...
            var otherBrand = "";
            foreach (var formKey in form.Keys)
                if (formKey.Equals($"product_manufacturer_otherbrand_{product.Id}", StringComparison.InvariantCultureIgnoreCase))
                {
                    otherBrand = form[formKey];
                    break;
                }
        ...
        public virtual int ParseSelectedProductAttribute(Product product, IFormCollection form)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            if (form == null)
            ...
            foreach (var formKey in form.Keys)
                if (formKey.Equals($"product_attribute_{product.Id}", StringComparison.InvariantCultureIgnoreCase))
                {
                    int.TryParse(form[formKey], out productAttributeId);
                    break;
                }
