...
namespace Zaraye.Services.Security
{
    ...
    public partial class PermissionService : IPermissionService
    {
        ...
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeSecurityDefaults.PermissionRecordsAllCacheKey, customerRoleId);
        ...
        //               where pr.Published
        //               orderby pr.DisplayOrder/*, pr.Category, pr.Name*/
        //               select pr;
        //    });

        //    return permissions;
        //}
        public virtual async Task<IList<PermissionRecord>> GetAllPermissionRecordsAsync(string categoryName = null)
        {
            var _storeContext = EngineContext.Current.Resolve<IStoreContext>();
            var permissions = await _permissionRecordRepository.GetAllAsync(query =>
            {
                var permission = from pr in query
                                 select pr;

                //if (_storeContext.GetCurrentStore().Id > 1)
                //    permission = permission.Where(p => p.Published && p.DisplayToStore);
                //else
                permission = permission.Where(p => p.Published);

                if (!string.IsNullOrWhiteSpace(categoryName))
                    permission = permission.Where(p => p.Category == categoryName);

                permission = permission.OrderBy(p => p.Name);
        ...
        public virtual async Task<int> GetPermissionRecordIdBySystemNameAsync(string systemName)
        {
            if (string.IsNullOrWhiteSpace(systemName))
                return 0;
            ...
            var permissionRecord = (await query.FirstOrDefaultAsync())?.Id ?? 0;
            return permissionRecord;
        }
        ...
        public virtual async Task<bool> AuthorizeAsync(string permissionRecordSystemName, int customerRoleId)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeSecurityDefaults.PermissionAllowedCacheKey, permissionRecordSystemName, customerRoleId);
        ...
        public virtual async Task<IPagedList<PermissionRecord>> GetPermissionRecordsByCustomerRoleIdAsync(int roleId, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = from pr in _permissionRecordRepository.Table
                        join prcrm in _permissionRecordCustomerRoleMappingRepository.Table on pr.Id equals prcrm
                            .PermissionRecordId
                        where prcrm.CustomerRoleId == roleId
                        orderby pr.DisplayOrder
                        select pr;

            return await query.ToPagedListAsync(pageIndex, pageSize);
        }

        public virtual async Task<PermissionRecordCustomerRoleMapping> GetMappingByPermissionRecordIdAndRoleIdAsync(int permissionId, int roleId)
        {
            var query = _permissionRecordCustomerRoleMappingRepository.Table;

            query = query.Where(x => x.PermissionRecordId == permissionId && x.CustomerRoleId == roleId);
