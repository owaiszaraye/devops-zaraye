    ...
    public partial class CookieAuthenticationService : IAuthenticationService
    {
        ...
        public virtual async Task SignInAsync(Customer customer, bool isPersistent)
        {
            ...
            if (!string.IsNullOrEmpty(customer.Username))
                claims.Add(new Claim(ClaimTypes.Name, customer.Username, ClaimValueTypes.String, ZarayeAuthenticationDefaults.ClaimsIssuer));

            if (!string.IsNullOrEmpty(customer.Email))
                claims.Add(new Claim(ClaimTypes.Email, customer.Email, ClaimValueTypes.Email, ZarayeAuthenticationDefaults.ClaimsIssuer));

            //create principal for the current authentication scheme
            var userIdentity = new ClaimsIdentity(claims, ZarayeAuthenticationDefaults.AuthenticationScheme);
            ...
            //sign in
            await _httpContextAccessor.HttpContext.SignInAsync(ZarayeAuthenticationDefaults.AuthenticationScheme, userPrincipal, authenticationProperties);
        ...
        public virtual async Task SignOutAsync()
        {
            ...
            await _httpContextAccessor.HttpContext.SignOutAsync(ZarayeAuthenticationDefaults.AuthenticationScheme);
        }
        ...
        public virtual async Task<Customer> GetAuthenticatedCustomerAsync()
        {
            ...
            var authenticateResult = await _httpContextAccessor.HttpContext.AuthenticateAsync(ZarayeAuthenticationDefaults.AuthenticationScheme);
            ...
            //try to get customer by username
            var usernameClaim = authenticateResult.Principal.FindFirst(claim => claim.Type == ClaimTypes.Name
                && claim.Issuer.Equals(ZarayeAuthenticationDefaults.ClaimsIssuer, StringComparison.InvariantCultureIgnoreCase));
            if (usernameClaim != null)
                customer = await _customerService.GetCustomerByUsernameAsync(usernameClaim.Value);
