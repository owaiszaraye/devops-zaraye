...
namespace Zaraye.Services.Common
{
    ...
    public partial class MaintenanceService : IMaintenanceService
    {
        ...
        public MaintenanceService(IZarayeFileProvider fileProvider)
        {
        ...
        {
            var path = _fileProvider.GetAbsolutePath(ZarayeCommonDefaults.DbBackupsPath);
        ...
        public virtual IList<string> GetAllBackupFiles()
        {
            ...
            if (!_fileProvider.DirectoryExists(path)) 
                throw new ZarayeException("Backup directory not exists");

            return _fileProvider.GetFiles(path, $"*.{ZarayeCommonDefaults.DbBackupFileExtension}")
        ...
        {
            return _fileProvider.Combine(GetBackupDirectoryPath(), $"database_{DateTime.Now:yyyy-MM-dd-HH-mm-ss}_{CommonHelper.GenerateRandomDigitCode(10)}.{ZarayeCommonDefaults.DbBackupFileExtension}");
        }
