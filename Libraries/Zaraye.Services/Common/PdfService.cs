...
namespace Zaraye.Services.Common
{
    ...
    public partial class PdfService : IPdfService
    {
        ...
        protected virtual async Task<AddressItem> GetBillingAddressAsync(Language lang, Order order)
        {
            ...
            if (order.ShippingStatus != ShippingStatus.ShippingNotRequired)
            {
                if (!order.PickupInStore)
                {
                    if (order.ShippingAddressId == null || await _addressService.GetAddressByIdAsync(order.ShippingAddressId.Value) is not Address shippingAddress)
                        throw new ZarayeException($"Shipping is required, but address is not available. Order ID = {order.Id}");
        ...
        /// </returns>
        public virtual async Task PrintOrderToPdfAsync(Stream stream, Order order, Language language = null, Store store = null)
        {
            if (order == null)
            ...
            var source = new InvoiceSource()
            {
                ...
                BillingAddress = await GetBillingAddressAsync(language, order),
                ...
                CheckoutAttributes = order.CheckoutAttributeDescription,
                Totals = await GetTotalsAsync(language, order),
        ...
        public virtual async Task PrintOrdersToPdfAsync(Stream stream, IList<Order> orders, Language language = null)
        {
            if (stream == null)
            ...
            foreach (var order in orders)
            {
                ...
                await using var pdfStream = new MemoryStream();
                await PrintOrderToPdfAsync(pdfStream, order, language, currentStore);
                pdfStream.Position = 0;
        ...
        /// </returns>
        public virtual async Task<string> SaveOrderPdfToDiskAsync(Order order, Language language = null)
        {
            ...
            await PrintOrderToPdfAsync(fileStream, order, language, store: null);
