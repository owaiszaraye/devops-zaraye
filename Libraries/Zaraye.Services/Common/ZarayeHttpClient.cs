...
namespace Zaraye.Services.Common
{
    ...
    /// </summary>
    public partial class ZarayeHttpClient
    {
        #region Fields
        ...
        public ZarayeHttpClient(AdminAreaSettings adminAreaSettings,
        ...
        {
            //configure client
            client.BaseAddress = new Uri("https://www.zaraye.co/");
            client.Timeout = TimeSpan.FromSeconds(5);
            client.DefaultRequestHeaders.Add(HeaderNames.UserAgent, $"zaraye-{ZarayeVersion.CURRENT_VERSION}");
        ...
        public virtual async Task<string> GetLicenseCheckDetailsAsync()
        {
            ...
            var url = string.Format(ZarayeCommonDefaults.ZarayeLicenseCheckPath,
                storeUrl,
                ZarayeVersion.FULL_VERSION,
        ...
        public virtual async Task<RssFeed> GetNewsRssAsync()
        {
            ...
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(ZarayeCommonDefaults.ZarayeNewsRssPath,
                ZarayeVersion.FULL_VERSION,
        ...
        {
            //prepare URL to request
            var url = string.Format(ZarayeCommonDefaults.ZarayeInstallationCompletedPath,
                ZarayeVersion.FULL_VERSION,
                _webHelper.IsLocalRequest(_httpContextAccessor.HttpContext.Request),
        ...
        public virtual async Task<string> GetExtensionsCategoriesAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(ZarayeCommonDefaults.ZarayeExtensionsCategoriesPath, language).ToLowerInvariant();
        ...
        public virtual async Task<string> GetExtensionsVersionsAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(ZarayeCommonDefaults.ZarayeExtensionsVersionsPath, language).ToLowerInvariant();
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(ZarayeCommonDefaults.ZarayeExtensionsPath,
