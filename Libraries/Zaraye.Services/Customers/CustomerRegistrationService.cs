    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual async Task<CustomerLoginResults> ValidateCustomerAsync(string usernameOrEmail, string password)
        {
            ...
            var selectedProvider = await _permissionService.AuthorizeAsync(StandardPermissionProvider.EnableMultiFactorAuthentication, customer)
                ? await _genericAttributeService.GetAttributeAsync<string>(customer, ZarayeCustomerDefaults.SelectedMultiFactorAuthenticationProviderAttribute)
                : null;
        ...
        public virtual async Task<CustomerRegistrationResult> RegisterCustomerAsync(CustomerRegistrationRequest request)
        {
            ...
            switch (request.PasswordFormat)
            {
                ...
                case PasswordFormat.Encrypted:
                    ...
                    break;
                case PasswordFormat.Hashed:
                    var saltKey = _encryptionService.CreateSaltKey(ZarayeCustomerServicesDefaults.PasswordSaltKeySize);
            ...
            var registeredRole = await _customerService.GetCustomerRoleBySystemNameAsync(ZarayeCustomerDefaults.RegisteredRoleName);
            if (registeredRole == null)
                throw new ZarayeException("'Registered' role could not be loaded");
            ...
            if (request.IsSupplier)
            {
                ...
                var sellerRole = await _customerService.GetCustomerRoleBySystemNameAsync(ZarayeCustomerDefaults.SupplierRoleName);
                if (sellerRole == null)
                    throw new ZarayeException("'Supplier' role could not be loaded");

                await _customerService.AddCustomerRoleMappingAsync(new CustomerCustomerRoleMapping { CustomerId = request.Customer.Id, CustomerRoleId = sellerRole.Id });
            }
            ...
            if (request.IsBuyer)
            {
                ...
                if (buyerRole == null)
                    throw new ZarayeException("'Buyer' role could not be loaded");

                await _customerService.AddCustomerRoleMappingAsync(new CustomerCustomerRoleMapping { CustomerId = request.Customer.Id, CustomerRoleId = buyerRole.Id });
            }
            ...
            if (request.IsBooker)
            {
                ...
                if (bookerRole == null)
                    throw new ZarayeException("'Booker' role could not be loaded");

                await _customerService.AddCustomerRoleMappingAsync(new CustomerCustomerRoleMapping { CustomerId = request.Customer.Id, CustomerRoleId = bookerRole.Id });
            }

            //remove from 'Guests' role            
            if (await _customerService.IsGuestAsync(request.Customer))
            {
                var guestRole = await _customerService.GetCustomerRoleBySystemNameAsync(ZarayeCustomerDefaults.GuestsRoleName);
        ...
        public virtual async Task SetEmailAsync(Customer customer, string newEmail, bool requireValidation)
        {
            ...
            if (newEmail == null)
                throw new ZarayeException("Email cannot be null");

            newEmail = newEmail.Trim();
            var oldEmail = customer.Email;

            if (!CommonHelper.IsValidEmail(newEmail))
                throw new ZarayeException(await _localizationService.GetResourceAsync("Account.EmailUsernameErrors.NewEmailIsNotValid"));

            if (newEmail.Length > 100)
                throw new ZarayeException(await _localizationService.GetResourceAsync("Account.EmailUsernameErrors.EmailTooLong"));

            var customer2 = await _customerService.GetCustomerByEmailAsync(newEmail);
            if (customer2 != null && customer.Id != customer2.Id)
                throw new ZarayeException(await _localizationService.GetResourceAsync("Account.EmailUsernameErrors.EmailAlreadyExists"));
            ...
            if (requireValidation)
            {
                ...
                await _genericAttributeService.SaveAttributeAsync(customer, ZarayeCustomerDefaults.EmailRevalidationTokenAttribute, Guid.NewGuid().ToString());
        ...
        public virtual async Task SetUsernameAsync(Customer customer, string newUsername)
        {
            ...
            if (newUsername.Length > ZarayeCustomerServicesDefaults.CustomerUsernameLength)
                throw new ZarayeException(await _localizationService.GetResourceAsync("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = await _customerService.GetCustomerByUsernameAsync(newUsername);
            if (user2 != null && customer.Id != user2.Id)
                throw new ZarayeException(await _localizationService.GetResourceAsync("Account.EmailUsernameErrors.UsernameAlreadyExists"));
