...
namespace Zaraye.Services.Customers
{
    ...
    public partial interface ICustomerService
    {
        ...
        // int affiliateId = 0, int vendorId = 0, int[] customerRoleIds = null, int buyerTypeId = 0, int supplierTypeId = 0,
       ...
       bool? isActive = null, bool? getOnlyBuyerOrders = null, bool? getOnlySupplierOrders = null, bool? getIszarayeTranspoter = null, int supportAgentId = 0, int parentId = 0, int? filterByCompanyOrEmployee = null,bool isPoc = false, string keywords = null);
        /// <summary>
        ...
        Task<string> GetCustomerFullNameAsync(int customerId);
        ...
        Task<bool> IsBookerAsync(Customer customer, bool onlyActiveCustomerRoles = true);

        Task<bool> IsDemandAsync(Customer customer, bool onlyActiveCustomerRoles = true);

        Task<bool> IsSupplyAsync(Customer customer, bool onlyActiveCustomerRoles = true);

        Task<bool> IsOperationsAsync(Customer customer, bool onlyActiveCustomerRoles = true);

        Task<bool> IsTrasnporterAsync(Customer customer, bool onlyActiveCustomerRoles = true);
        

        Task<bool> IsOrderBookerDemandAsync(Customer customer, bool onlyActiveCustomerRoles = true);
         Task<bool> IsDemandAssociateAsync(Customer customer, bool onlyActiveCustomerRoles = true);
         Task<bool> IsSupplyAssociateAsync(Customer customer, bool onlyActiveCustomerRoles = true);
         Task<bool> IsGroundOperationsAsync(Customer customer, bool onlyActiveCustomerRoles = true);
         Task<bool> IsOPerationAssociateAsync(Customer customer, bool onlyActiveCustomerRoles = true);
         Task<bool> IsOPerationLeadAsync(Customer customer, bool onlyActiveCustomerRoles = true);
         Task<bool> IsBusinessLeadAsync(Customer customer, bool onlyActiveCustomerRoles = true);

         Task<bool> IsControlTowerAsync(Customer customer, bool onlyActiveCustomerRoles = true);

        Task<bool> IsBrokerAsync(Customer customer, bool onlyActiveCustomerRoles = true);
        Task<bool> IsThirdPartyLedgerAsync(Customer customer, bool onlyActiveCustomerRoles = true);
        ...
        Task<bool> IsLiveOpsAsync(Customer customer, bool onlyActiveCustomerRoles = true);
        /// <summary>
        ...
        Task<bool> IsBuyerAsync(Customer customer, bool onlyActiveCustomerRoles = true);

        Task<bool> IsSupplierAsync(Customer customer, bool onlyActiveCustomerRoles = true);

        Task<bool> IsFinanceAsync(Customer customer, bool onlyActiveCustomerRoles = true);
        
        Task<bool> IsBusinessHeadAsync(Customer customer, bool onlyActiveCustomerRoles = true);

        Task<bool> IsFinanceHeadAsync(Customer customer, bool onlyActiveCustomerRoles = true);

        Task<bool> IsOpsHeadAsync(Customer customer, bool onlyActiveCustomerRoles = true);
        ...
        Task DeleteUserTypeAsync(UserType userType);

        Task<UserType> GetUserTypeByIdAsync(int userTypeId);

        Task InsertUserTypeAsync(UserType userType);

        Task UpdateUserTypeAsync(UserType userType);

        Task<IPagedList<UserType>> GetAllUserTypesAsync(string type = "", int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false);
        ...
        Task InsertTransporterVehicleMappingAsync(TransporterVehicleMapping TransporterVehicle);
        Task UpdateTransporterVehicleMappingAsync(TransporterVehicleMapping TransporterVehicle);
        Task<TransporterVehicleMapping> GetTransporterVehicleMappingByIdAsync(int TransporterVehicleId);
        Task<IPagedList<TransporterVehicleMapping>> GetTransporterVehicleByTransporterIdAsync(int TransporterId, int pageIndex = 0, int pageSize = int.MaxValue);
        Task DeleteTransporterVehicleMappingAsync(TransporterVehicleMapping TransporterVehicle);
        Task<TransporterVehicleMapping> GetTransporterVehicleByVehicleNumberAsync(string vehiclenumber);
        Task<TransporterVehicleMapping> GetTransporterVehiclesByVehicleIdAsync(int vehicleId, int transporterId);

        #endregion

        #region TranspoterCost

        Task DeleteTranspoterCostAsync(TransporterCost transporterCost);

        Task<TransporterCost> GetTranspoterCostByIdAsync(int transporterCostId);

        Task<TransporterCost> GetTranspoterCostByTransporterIdAsync(int transporterId);
        decimal GetTotalMonthlyCostByTransporterId(int transporterId);

        Task InsertTranspoterCostAsync(TransporterCost transporterCost);

        Task UpdateTranspoterCostAsync(TransporterCost transporterCost);

        Task<IPagedList<TransporterCost>> GetTransporterCostByTransporterIdAsync(int TransporterId, int pageIndex = 0, int pageSize = int.MaxValue);

        Task<VehiclePortfolio> GetVehiclePortfolioByTransporterVehicleMappingIdAsync(int transporterId, int transporterVehicleMappingId);
        #endregion

        #region Customer Industry Mappings

        Task AddCustomerIndustryMappingAsync(CustomerIndustryMapping customerIndustryMapping);
        Task DeleteCustomerIndustryMappingAsync(CustomerIndustryMapping customerIndustryMapping);
        Task RemoveCustomerIndustryMappingAsync(Customer customer, Category industry);

        Task<int[]> GetCustomerIndustryIdsAsync(Customer customer, bool showHidden = false);
        Task<IList<CustomerIndustryMapping>> GetCustomerIndustriesAsync(Customer customer, bool showHidden = false);
        Task<IPagedList<CustomerIndustryMapping>> GetCustomerIndustryMappingsByIndustryIdAsync(int industryId,
           int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false);
        CustomerIndustryMapping FindCustomerIndustryMapping(IList<CustomerIndustryMapping> source, int industryId, int customerId);

        #endregion

        #region Vehicle portfolio

        Task DeleteVehiclePortfolioAsync(VehiclePortfolio vehiclePortfolio);
        Task<VehiclePortfolio> GetVehiclePortfolioByIdAsync(int vehiclePortfolioId);
        Task InsertVehiclePortfolioAsync(VehiclePortfolio vehiclePortfolio);
        Task UpdateVehiclePortfolioAsync(VehiclePortfolio vehiclePortfolio);
        Task<IPagedList<VehiclePortfolio>> GetAllVehiclePortfolioAsync(string name = "",
        ...
        Task InsertBankDetailAsync(BankDetail bankDetail);
        Task UpdateBankDetailAsync(BankDetail bankDetail);
        Task DeleteBankDetailAsync(BankDetail bankDetail);
        Task<BankDetail> GetBankDetailByIdAsync(int id);
        Task<IPagedList<BankDetail>> GetAllBankDetailAsync(int customerId = 0, bool showHidden = false, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false);
        Task<BankDetail> GetBankDetailByBAAAsync(string bankname, string accountNumber, string accountTitle);

        #endregion

        #region Applied Credit Customer

        Task InsertAppliedCreditCustomerAsync(AppliedCreditCustomer appliedCredit);

        Task UpdateAppliedCreditCustomerAsync(AppliedCreditCustomer appliedCredit);

        Task DeleteAppliedCreditCustomerAsync(AppliedCreditCustomer appliedCredit);
        
        Task<AppliedCreditCustomer> GetAppliedCreditCustomerByIdAsync(int id);

        Task<AppliedCreditCustomer> GetAppliedCreditCustomerByCustomerIdAsync(int customerId);

        Task<IPagedList<AppliedCreditCustomer>> GetAllAppliedCreditCustomerAsync(int customerId = 0, string fullname = "",
        ...
        Task InsertRejectedReasonAsync(OnlineLeadRejectReason rejectReason);

        Task UpdateRejectedReasonAsync(OnlineLeadRejectReason rejectReason);

        Task<OnlineLeadRejectReason> GetRejectedReasonByIdAsync(int rejectReasonId);

        Task DeleteRejectedReasonAsync(OnlineLeadRejectReason rejectReason);

        Task<IPagedList<OnlineLeadRejectReason>> GetAllRejectedReasonAsync(string name = "", int type = 0, bool? overridePublished = null,
             bool showHidden = false, int pageIndex = 0, int pageSize = int.MaxValue);

        #endregion
        Task InsertBankStatementAsync(BankStatement bankStatement);
    }
}