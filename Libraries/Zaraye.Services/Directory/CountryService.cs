...
namespace Zaraye.Services.Directory
{
    ...
    public partial class CountryService : ICountryService
    {
        ...
        /// </returns>
        public virtual async Task<IList<Country>> GetAllCountriesAsync(int languageId = 0, bool showHidden = false, bool? isAppPublished = null)
        {
            var store = await _storeContext.GetCurrentStoreAsync();
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeDirectoryDefaults.CountriesAllCacheKey, languageId,
                showHidden, store);
            ...
            return await _staticCacheManager.GetAsync(key, async () =>
            {
                var countries = await _countryRepository.GetAllAsync(async query =>
                {
                    ...
                    {
                        query = query.Where(c => c.PublishedOnApp);
                    }
                    else
                    {
                        if (!showHidden)
        ...
        public virtual async Task<Country> GetCountryByTwoLetterIsoCodeAsync(string twoLetterIsoCode)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeDirectoryDefaults.CountriesByTwoLetterCodeCacheKey, twoLetterIsoCode);
        ...
        public virtual async Task<Country> GetCountryByThreeLetterIsoCodeAsync(string threeLetterIsoCode)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeDirectoryDefaults.CountriesByThreeLetterCodeCacheKey, threeLetterIsoCode);
