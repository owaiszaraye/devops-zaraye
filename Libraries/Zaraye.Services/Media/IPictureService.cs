...
namespace Zaraye.Services.Media
{
    ...
    public partial interface IPictureService
    {
        ...
        Task DeleteAppSlidersAsync(AppSlider appSlider);

        Task<AppSlider> GetAppSliderByIdAsync(int appSliderId);

        Task<IPagedList<AppSlider>> GetAllAppSlidersAsync(bool? published = null,
            string title = "", bool showHidden = false, int pageIndex = 0, int pageSize = int.MaxValue);

        Task InsertAppSlidersAsync(AppSlider appSlider);

        Task UpdateAppSlidersAsync(AppSlider appSlider);

        #endregion

        #region Generic Picture Mapping
        Task<GenericPicture> GetGenericPictureByIdAsync(int genericPictureId);
        Task DeleteGenericPictureAsync(GenericPicture picture);
        Task<IList<GenericPicture>> GetGenericPicturesByEntityIdAndEntityTypeIdAsync(int entityId, int entityTypeId);
        Task InsertGenericPictureAsync(GenericPicture genericPicture);
        Task UpdateGenericPictureAsync(GenericPicture genericPicture);
        Task<IPagedList<GenericPicture>> SearchGenericPicturesAsync(
            int pageIndex = 0,
