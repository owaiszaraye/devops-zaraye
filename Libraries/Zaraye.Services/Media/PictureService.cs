...
namespace Zaraye.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        private readonly IRepository<AppSlider> _appSliderRepository;
        private IDownloadService downloadService;
        private IHttpContextAccessor httpContextAccessor;
        private ILogger logger;
        private IZarayeFileProvider fileProvider;
        private IProductAttributeParser productAttributeParser;
        private IRepository<Picture> pictureRepository;
        private IRepository<PictureBinary> pictureBinaryRepository;
        private IRepository<ProductPicture> productPictureRepository;
        ...
        public PictureService(IDownloadService downloadService,
            ...
            IRepository<AppSlider> appSliderRepository,
            ...
            IStoreService storeService)
        {
            ...
            _appSliderRepository = appSliderRepository;
            _storeContext = storeContext;
            _storeService = storeService;
        }

        public PictureService(IDownloadService downloadService, IHttpContextAccessor httpContextAccessor, ILogger logger, IZarayeFileProvider fileProvider, IProductAttributeParser productAttributeParser, IRepository<Picture> pictureRepository, IRepository<PictureBinary> pictureBinaryRepository, IRepository<ProductPicture> productPictureRepository, ISettingService settingService, IUrlRecordService urlRecordService, IWebHelper webHelper, MediaSettings mediaSettings)
        {
            ...
            this.pictureBinaryRepository = pictureBinaryRepository;
        ...
        protected virtual async Task DeletePictureThumbsAsync(Picture picture)
        {
            ...
            var currentFiles = _fileProvider.GetFiles(_fileProvider.GetAbsolutePath(ZarayeMediaDefaults.ImageThumbsPath), filter, false);
        ...
        protected virtual Task<string> GetThumbLocalPathAsync(string thumbFileName)
        {
            ...
            var thumbsDirectoryPath = _fileProvider.GetAbsolutePath($"{store.Url}{ZarayeMediaDefaults.ImageThumbsPath}");

            if (_mediaSettings.MultipleThumbDirectories)
            {
                //get the first two letters of the file name
                var fileNameWithoutExtension = _fileProvider.GetFileNameWithoutExtension(thumbFileName);
                if (fileNameWithoutExtension != null && fileNameWithoutExtension.Length > ZarayeMediaDefaults.MultipleThumbDirectoriesLength)
                {
                    var subDirectoryName = fileNameWithoutExtension[0..ZarayeMediaDefaults.MultipleThumbDirectoriesLength];
                    thumbsDirectoryPath = _fileProvider.GetAbsolutePath(ZarayeMediaDefaults.ImageThumbsPath, subDirectoryName);
        ...
        protected virtual async Task SaveThumbAsync(string thumbFilePath, string thumbFileName, string mimeType, byte[] binary)
        {
            ...
            var thumbsDirectoryPath = _fileProvider.GetAbsolutePath(ZarayeMediaDefaults.ImageThumbsPath);
            _fileProvider.CreateDirectory(thumbsDirectoryPath);
            ...
            string storeLocation = null)
        {
            ...
            {
                PictureType.Avatar => await _settingService.GetSettingByKeyAsync("Media.Customer.DefaultAvatarImageName", ZarayeMediaDefaults.DefaultAvatarFileName),
                _ => await _settingService.GetSettingByKeyAsync("Media.DefaultImageName", ZarayeMediaDefaults.DefaultImageFileName),
            };
        ...
        public virtual async Task<string> GetPictureUrlAsync(int pictureId,
            ...
            PictureType defaultPictureType = PictureType.Entity)
        {
            var storeUrl = _storeContext.GetCurrentStore().Url;
            var picture = await GetPictureByIdAsync(pictureId);
            return (await GetPictureUrlAsync(picture, targetSize, showDefaultPicture, storeLocation == null ? storeUrl : storeLocation, defaultPictureType)).Url;
        }
        ...
        {
            if (picture == null)
                return showDefaultPicture ? (await GetDefaultPictureUrlAsync(0, defaultPictureType, storeLocation), null) : (string.Empty, (Picture)null);
        ...
        public virtual async Task<Picture> InsertPictureAsync(IFormFile formFile, string defaultFileName = "", string virtualPath = "")
        {
            ...
            //condition to cater the # special character to cure the image not found issue
        ...
        #region App Slider Methods

        public virtual async Task DeleteAppSlidersAsync(AppSlider appSlider)
        {
            await _appSliderRepository.DeleteAsync(appSlider);
        }

        public virtual async Task<AppSlider> GetAppSliderByIdAsync(int appSliderId)
        {
            return await _appSliderRepository.GetByIdAsync(appSliderId, cache => default);
        }

        public virtual async Task<IPagedList<AppSlider>> GetAllAppSlidersAsync(bool? published = null,
            string title = "", bool showHidden = false, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _appSliderRepository.Table;
            ...
            var sliders = await query.ToPagedListAsync(pageIndex, pageSize);
            return sliders;
        }

        public virtual async Task InsertAppSlidersAsync(AppSlider appSlider)
        {
            await _appSliderRepository.InsertAsync(appSlider);
        }

        public virtual async Task UpdateAppSlidersAsync(AppSlider appSlider)
        {
            await _appSliderRepository.UpdateAsync(appSlider);
        }

        #endregion

        #region Generic Picture Mapping

        public virtual async Task<GenericPicture> GetGenericPictureByIdAsync(int genericPictureId)
        {
            var _genericPictureRepository = EngineContext.Current.Resolve<IRepository<GenericPicture>>();

            return await _genericPictureRepository.GetByIdAsync(genericPictureId);
        }
        public virtual async Task DeleteGenericPictureAsync(GenericPicture picture)
        {
            var _genericPictureRepository = EngineContext.Current.Resolve<IRepository<GenericPicture>>();
            if (picture == null)
                throw new ArgumentNullException(nameof(picture));
            ...
            //delete from database
            await _genericPictureRepository.DeleteAsync(picture);
        }
        public virtual async Task<IList<GenericPicture>> GetGenericPicturesByEntityIdAndEntityTypeIdAsync(int entityId, int entityTypeId)
        {
            var _genericPictureRepository = EngineContext.Current.Resolve<IRepository<GenericPicture>>();
            if (entityId == 0)
                return new List<GenericPicture>();

            var query = from gp in _genericPictureRepository.Table
                        where gp.EntityId == entityId && gp.EntityType == entityTypeId
                        select gp;
        ...
        public virtual async Task<IPagedList<GenericPicture>> SearchGenericPicturesAsync(
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            int entityId = 0, int entityTypeId = 0)
        {
            var _genericPictureRepository = EngineContext.Current.Resolve<IRepository<GenericPicture>>();
            ...
            var query = _genericPictureRepository.Table;

            if (entityId > 0)
                query = query.Where(x => x.EntityId == entityId);

            if (entityTypeId > 0)
                query = query.Where(x => x.EntityType == entityTypeId);

            return await query.OrderBy(x => x.Id).ToPagedListAsync(pageIndex, pageSize);
        }
        public virtual async Task InsertGenericPictureAsync(GenericPicture genericPicture)
        {
            var _genericPictureRepository = EngineContext.Current.Resolve<IRepository<GenericPicture>>();
            if (genericPicture == null)
                throw new ArgumentNullException(nameof(genericPicture));

            //delete from database
            await _genericPictureRepository.InsertAsync(genericPicture);
        }

        public virtual async Task UpdateGenericPictureAsync(GenericPicture genericPicture)
        {
            var _genericPictureRepository = EngineContext.Current.Resolve<IRepository<GenericPicture>>();
            if (genericPicture == null)
                throw new ArgumentNullException(nameof(genericPicture));
            ...
            //delete from database
            await _genericPictureRepository.UpdateAsync(genericPicture);
        }
        #endregion
    }
}