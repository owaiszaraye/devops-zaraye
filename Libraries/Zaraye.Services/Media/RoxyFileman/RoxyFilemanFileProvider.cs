...
namespace Zaraye.Services.Media.RoxyFileman
{
    /// <summary>
    ...
    public class RoxyFilemanFileProvider : PhysicalFileProvider, IRoxyFilemanFileProvider
    {
        ...
        protected IZarayeFileProvider _zarayeFileProvider;
        ...
        public RoxyFilemanFileProvider(IZarayeFileProvider zarayeFileProvider) : base(zarayeFileProvider.Combine(zarayeFileProvider.WebRootPath, ZarayeRoxyFilemanDefaults.DefaultRootDirectory))
        {
            _zarayeFileProvider = zarayeFileProvider;
        }

        public RoxyFilemanFileProvider(IZarayeFileProvider defaultFileProvider, MediaSettings mediaSettings) : this(defaultFileProvider)
        {
        ...
        public virtual async Task<RoxyFilemanConfig> GetOrCreateConfigurationAsync(string pathBase, string lang)
        {
            ...
            var filePath = _zarayeFileProvider.GetAbsolutePath(ZarayeRoxyFilemanDefaults.ConfigurationFile);

            //create file if not exists
            _zarayeFileProvider.CreateFile(filePath);

            //try to read existing configuration
            var existingText = await _zarayeFileProvider.ReadAllTextAsync(filePath, Encoding.UTF8);
            ...
            {
                FILES_ROOT = existingConfiguration?.FILES_ROOT ?? ZarayeRoxyFilemanDefaults.DefaultRootDirectory,
                ...
                FORBIDDEN_UPLOADS = existingConfiguration?.FORBIDDEN_UPLOADS ?? string.Join(" ", ZarayeRoxyFilemanDefaults.ForbiddenUploadExtensions),
