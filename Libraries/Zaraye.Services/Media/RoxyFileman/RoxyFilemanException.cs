...
namespace Zaraye.Services.Media.RoxyFileman
{
    ...
    public class RoxyFilemanException : Exception
    {
        ...
        protected static string GetLocalizedMessage(string key)
        {
            ...
            var languageFile = fileProvider.GetAbsolutePath($"{ZarayeRoxyFilemanDefaults.LanguageDirectory}/{roxyConfig.LANG}.json");

            if (!fileProvider.FileExists(languageFile))
                languageFile = fileProvider.GetAbsolutePath($"{ZarayeRoxyFilemanDefaults.LanguageDirectory}/en.json");
