...
namespace Zaraye.Services.Blogs
{
    ...
    public partial class BlogService : IBlogService
    {
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false, string title = null)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeCatalogDefaults.AllBlogPostCacheKey, $"pagesize{pageSize}", $"pageindex{pageIndex}");
            var blogs = await _staticCacheManager
             .GetAsync(key, async () =>
             {
        ...
        {
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeBlogsDefaults.BlogTagsCacheKey, languageId, storeId, showHidden);
        ...
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeBlogsDefaults.BlogCommentsNumberCacheKey);

            var Comments = await _staticCacheManager
               .GetAsync(key, async () =>
               {
        ...
        public virtual async Task<int> GetBlogCommentsCountAsync(BlogPost blogPost, int storeId = 0, bool? isApproved = null)
        {
            ...
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ZarayeBlogsDefaults.BlogCommentsNumberCacheKey, blogPost, storeId, isApproved);
