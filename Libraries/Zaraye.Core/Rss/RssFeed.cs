...
namespace Zaraye.Core.Rss
{
    ...
    public partial class RssFeed
    {
        ...
        {
            Title = new XElement(ZarayeRssDefaults.Title, title);
            Description = new XElement(ZarayeRssDefaults.Description, description);
            Link = new XElement(ZarayeRssDefaults.Link, link);
            LastBuildDate = new XElement(ZarayeRssDefaults.LastBuildDate, lastBuildDate.ToString("r"));
        }
        ...
        public static async Task<RssFeed> LoadAsync(Stream stream)
        {
            try
            {
                ...
                var channel = document.Root?.Element(ZarayeRssDefaults.Channel);

                if (channel == null)
                    return null;

                var title = channel.Element(ZarayeRssDefaults.Title)?.Value ?? string.Empty;
                var description = channel.Element(ZarayeRssDefaults.Description)?.Value ?? string.Empty;
                var link = new Uri(channel.Element(ZarayeRssDefaults.Link)?.Value ?? string.Empty);
                var lastBuildDateValue = channel.Element(ZarayeRssDefaults.LastBuildDate)?.Value;
                ...
                foreach (var item in channel.Elements(ZarayeRssDefaults.Item))
                {
        ...
        public string GetContent()
        {
            ...
            var root = new XElement(ZarayeRssDefaults.RSS, new XAttribute("version", "2.0"));
            var channel = new XElement(ZarayeRssDefaults.Channel,
