...
namespace Zaraye.Core.Rss
{
    ...
    public partial class RssItem
    {
        ...
        public RssItem(string title, string content, Uri link, string id, DateTimeOffset pubDate)
        {
            ...
            Content = new XElement(ZarayeRssDefaults.Description, content);
            Link = new XElement(ZarayeRssDefaults.Link, link);
            Id = new XElement(ZarayeRssDefaults.Guid, new XAttribute("isPermaLink", false), id);
            PubDate = new XElement(ZarayeRssDefaults.PubDate, pubDate.ToString("r"));
        }
        ...
        {
            var title = item.Element(ZarayeRssDefaults.Title)?.Value ?? string.Empty;
            var content = item.Element(ZarayeRssDefaults.Content)?.Value ?? string.Empty;
            if (string.IsNullOrEmpty(content))
                content = item.Element(ZarayeRssDefaults.Description)?.Value ?? string.Empty;
            var link = new Uri(item.Element(ZarayeRssDefaults.Link)?.Value ?? string.Empty);
            var pubDateValue = item.Element(ZarayeRssDefaults.PubDate)?.Value;
            var pubDate = pubDateValue == null ? DateTimeOffset.Now : DateTimeOffset.ParseExact(pubDateValue, "r", null);
            var id = item.Element(ZarayeRssDefaults.Guid)?.Value ?? string.Empty;
        ...
        {
            var element = new XElement(ZarayeRssDefaults.Item, Id, Link, Title, Content);
